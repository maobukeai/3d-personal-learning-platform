...
<template>
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-3 lg:gap-6 items-start w-full">
    ...
    >
      <div
        class="flex flex-row lg:flex-row items-center lg:justify-between justify-start pb-0 lg:pb-2 border-b-0 lg:border-b border-slate-200 dark:border-slate-800 shrink-0 gap-1.5 lg:w-full"
      >
        <span
          class="text-xs font-bold text-slate-700 dark:text-slate-200 flex items-center gap-1.5"
        >
      ...
      >
        ...
        >
          <span class="truncate max-w-[100px] lg:max-w-none">{{
            cat === 'all'
              ? t('tools.googleWarming.allCategories')
              : cat === '未分类'
                ? t('tools.googleWarming.uncategorized')
                : cat
          }}</span>
          <span
            :class="[
    ...
    <div class="lg:col-span-9 gw-card !p-3 sm:!p-4 flex flex-col gap-3 min-h-[500px]">
      ...
      <div class="flex flex-wrap gap-2 items-center justify-between min-h-[32px]">
        ...
        <div
          ...
          :class="{ 'sm:ml-auto': selectedAccountIds.length === 0 }"
        >
          <input
            v-model="searchQuery"
            type="text"
        ...
        >
          ...
          <el-table-column label="邮箱账号" min-width="210">
            <template #default="{ row }">
              ...
              >
                <GripVertical
                  class="w-3.5 h-3.5 opacity-40 shrink-0 cursor-grab active:cursor-grabbing"
                ...
                >
                <button
                  class="hover:text-violet-600 dark:hover:text-violet-400 p-0.5"
                  style="color: var(--text-muted)"
                  title="复制邮箱"
                ...
                >
                  <template #reference>
                    <button
                      class="text-indigo-500 hover:text-indigo-400 p-0.5"
                      title="点击获取2FA验证码"
                      @click.stop
                    >
                  ...
                  <div class="p-2 space-y-2 text-xs">
                    ...
                    <div
                      class="flex items-center justify-between bg-slate-100 dark:bg-slate-950/50 p-2 rounded border border-slate-200 dark:border-slate-800"
                    >
            ...
            <template #default="{ row }">
              <div v-if="row.recoveryEmail" class="flex items-center gap-1 justify-between">
                <span
                  ...
                  :title="row.recoveryEmail"
                  >{{ row.recoveryEmail }}</span
                >
                <button
                  class="hover:text-violet-600 dark:hover:text-violet-400 p-0.5"
                  style="color: var(--text-muted)"
                  title="复制辅助邮箱"
                  @click="copyText(row.recoveryEmail)"
                >
                  <Copy class="w-3.5 h-3.5" />
                </button>
              </div>
            ...
            <template #default="{ row }">
              <div v-if="row.backupCodes" class="flex items-center gap-1 justify-between">
                <span
                  class="truncate max-w-[120px]"
                  style="color: var(--text-secondary)"
                  :title="row.backupCodes"
                  >{{ row.backupCodes }}</span
                >
                <button
                  class="hover:text-violet-600 dark:hover:text-violet-400 p-0.5"
                  style="color: var(--text-muted)"
                  title="复制备用密码"
                  @click="copyText(row.backupCodes)"
                >
                  <Copy class="w-3.5 h-3.5" />
                </button>
              </div>
          ...
          <el-table-column label="操作" width="90" align="right">
            <template #default="{ row }">
              <div class="flex items-center justify-end gap-1">
                <button
                  class="hover:text-violet-600 dark:hover:text-violet-400 p-1 transition-colors cursor-pointer"
                  style="color: var(--text-muted)"
                  @click="emit('edit', row)"
                >
                ...
                <button
                  class="hover:text-red-600 dark:hover:text-red-400 p-1 transition-colors cursor-pointer"
                  style="color: var(--text-muted)"
    ...
    <div class="space-y-3">
      ...
      >
        <table class="w-full text-xs text-left">
          <thead
            class="text-xs border-b font-medium"
            style="
          ...
          >
            ...
            >
              <td
                class="p-2.5 font-medium truncate max-w-[180px]"
                style="color: var(--text-primary)"
                :title="cat"
              >
                {{ cat }}
              ...
              <td
                class="p-2.5 text-center font-semibold font-mono"
                style="color: var(--text-muted)"
              >
                {{ getCategoryCount(cat) }}
              </td>
