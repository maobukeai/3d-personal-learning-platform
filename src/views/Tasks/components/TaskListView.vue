...
import { getTagClass, parseTags } from '@/utils/tags';
import {
...
const handleSubtaskUpdateInList = (task: Task, fields: Record<string, unknown>) => {
  if (task.isSubtask && task.parentId) {
    emit('update-subtask', task.parentId, task.subtaskIndex ?? 0, fields);
    return true;
  }
  return false;
};
...
<template>
  <div class="mobile-adaptive flex-1 overflow-y-auto p-1 sm:p-4 scrollbar-hide">
    <div class="w-full max-w-none space-y-8">
      ...
      >
        <!-- Project Card Header -->
        <div
          class="mobile-row flex items-center justify-between px-1 sm:px-2 py-0.5 select-none cursor-pointer group/proj"
          ...
          >
            ...
            <div v-show="!isGroupCollapsed(proj.id, col.id)" class="overflow-x-auto">
              ...
              >
                ...
                >
                  <div
                    class="task-table-grid grid grid-cols-12 px-1.5 sm:px-4 py-1.5 sm:py-2.5 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors items-center text-[10.5px] sm:text-xs group cursor-pointer"
                  ...
                  >
                    ...
                    >
                      <span
                        v-if="task.dueDate"
                        :class="
                          isOverdue(task.dueDate, task.status) ? 'text-rose-500' : 'text-slate-400'
                        "
                      >
                        <span class="hidden sm:inline">{{
                          new Date(task.dueDate).toLocaleDateString()
                        }}</span>
                        <span class="sm:hidden">{{ formatShortDate(task.dueDate) }}</span>
                      </span>
