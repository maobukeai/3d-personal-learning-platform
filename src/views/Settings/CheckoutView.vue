...
<template>
  <div class="min-h-screen bg-[var(--bg-app)] flex items-center justify-center p-6">
    <div
      class="w-full max-w-lg bg-[var(--bg-card)] rounded-[2.5rem] border border-[var(--border-base)] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-500"
    >
      <!-- Top Banner -->
      <div class="bg-accent p-8 text-white relative overflow-hidden">
        <div class="relative z-10">
          <button
            class="p-2 hover:bg-white/10 rounded-full transition-colors mb-4"
            @click="router.back()"
          >
      ...
      <div class="p-8 space-y-8">
        <div class="flex items-center justify-between">
          <div>
            ...
            <h2 class="text-lg font-black text-[var(--text-primary)]">
              {{ planName || '订阅计划' }}
            </h2>
          </div>
        ...
        <!-- Payment Methods -->
        <div class="space-y-4">
          <p class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">
            选择支付方式
          </p>
            ...
            <label
              ...
              :class="
                payMethod === 'WECHAT'
                  ...
                  : 'border-[var(--border-base)] hover:border-emerald-300'
              "
            >
              <input
                v-model="payMethod"
              ...
              />
              <div
                class="w-10 h-10 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 flex items-center justify-center"
              >
        ...
        <div
          class="flex flex-col items-center justify-center py-6 bg-slate-50 dark:bg-white/5 rounded-3xl border border-dashed border-[var(--border-base)]"
        >
        ...
        >
          ...
          <template v-else> 立即支付 ￥{{ amount }} </template>
        </button>

        <div class="flex items-center justify-center gap-2">
          <AlertCircle class="w-3.5 h-3.5 text-slate-300" />
          <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
            Secure Checkout Powered by Alipay
          </p>
        </div>
      </div>
    </div>
