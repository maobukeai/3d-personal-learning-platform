    ...
    <div v-else class="gw-card space-y-4 lg:space-y-6 !p-3 lg:!p-6">
      ...
      <div class="gw-account-detail-card flex items-center gap-4 py-2.5 px-3">
        ...
        <div class="flex items-center gap-1.5 shrink-0">
          <button @click="emit('edit', selectedAccount)" class="gw-icon-action-btn cursor-pointer" title="编辑">
            <Edit class="w-3.5 h-3.5" />
          </button>
          <button @click="emit('delete', selectedAccount)" class="gw-icon-danger-btn cursor-pointer" title="删除">
      ...
      <div class="pt-4 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center">
        <div class="text-[11px] text-slate-500">遇到谷歌关联地区风控？可以使用改区申诉理由助手生成专业的中英文陈述。</div>
        <button
