...
export const getCourseById = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const totalDuration = course.lessons.reduce((sum: number, l: any) => sum + (l.duration || 0), 0);
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const course = await tx.course.create({
        data: { title, description, thumbnail, categoryId: categoryId || null, difficulty: difficulty || 'BEGINNER' }
      });
...
export const enrollInCourse = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if ((error as any).code === 'P2002') {
      return res.status(400).json({ error: 'You are already enrolled in this course in this workspace' });
    }
    res.status(500).json({ error: 'Internal server error' });
  }
};
...
export const getMyEnrollments = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const enrollmentsWithRating = enrollments.map(e => {
      const avgRating = e.course.reviews.length > 0
        ? e.course.reviews.reduce((sum: number, r: any) => sum + r.rating, 0) / e.course.reviews.length
        : 0;
