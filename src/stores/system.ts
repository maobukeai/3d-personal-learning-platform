    ...
    async fetchSettings() {
      try {
        ...
        this.settings = {
          ...
          MATERIAL_CATEGORIES: Array.isArray(data.MATERIAL_CATEGORIES)
            ? data.MATERIAL_CATEGORIES
            : typeof data.MATERIAL_CATEGORIES === 'string'
              ? JSON.parse(data.MATERIAL_CATEGORIES)
              : ['全部材料', '金属', '木纹', '石材', '织物', '程序化', '玻璃', '其他'],
          ...
          ALLOWED_FILE_TYPES: Array.isArray(data.ALLOWED_FILE_TYPES)
            ...
            : typeof data.ALLOWED_FILE_TYPES === 'string'
              ? JSON.parse(data.ALLOWED_FILE_TYPES)
