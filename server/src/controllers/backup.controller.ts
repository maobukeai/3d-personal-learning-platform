...
export const saveBackupConfig = async (
  ...
  const userId = request.userId as string;
  const { url, username, password, dir, retentionDays } = request.body as {
    url?: string;
    username?: string;
    password?: string;
    dir?: string;
...
export const testBackupConfig = async (
  ...
  const userId = request.userId as string;
  const { url, username, password, dir } = request.body as {
    url?: string;
    username?: string;
    password?: string;
    dir?: string;
  ...
  if (!url || !username || !decryptedPassword) {
    reply.status(400).send({ error: '服务地址、账号和密码不能为空' });
    return;
  }
  ...
  reply.send({ success: true, message: 'WebDAV 服务连接成功！' });
};
 ...
 */
export const runBackup = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
  const userId = request.userId as string;
  const { categories } = request.body as { categories?: string[] }; // e.g. ['notes', 'microsoftEmail']

  if (!categories || !Array.isArray(categories) || categories.length === 0) {
    reply.status(400).send({ error: '请选择要备份的分类' });
    return;
  }

  const config = await getWebDAVConfig(userId);
  if (!config.url || !config.username || !config.password) {
    reply.status(400).send({ error: '请先配置 WebDAV 服务设置' });
    return;
  }
  ...
  // P5：ZIP CREATION — 纯内存 Buffer 化，不再写入 tmpdir（铁律二·1 零本地 IO）
  ...
  // 使用 PassThrough 流收集 archiver 输出到内存 Buffer，避免磁盘临时文件
  ...
  // UPLOAD TO WEBDAV — 直接从内存 Buffer 上传，无需读取临时文件
  ...
  reply.send({ success: true, filename, message: '备份成功并已上传至云端！' });
};

/**
 * List backups on WebDAV
 */
export const listBackups = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
  const userId = request.userId as string;
  const config = await getWebDAVConfig(userId);
  if (!config.url || !config.username || !config.password) {
    reply.send([]);
 ...
 */
export const deleteBackup = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
  const userId = request.userId as string;
  const { filename } = request.params as { filename: string };

  if (typeof filename !== 'string') {
    reply.status(400).send({ error: '文件名错误' });
    return;
  }
  ...
  reply.send({ success: true, message: '备份已成功从云端删除' });
};
...
export const restoreBackup = async (
  ...
  const userId = request.userId as string;
  const { filename, restoreCategories } = request.body as {
    filename?: string;
    restoreCategories?: string[];
  }; // restoreCategories: array of keys to restore

  if (!filename) {
    reply.status(400).send({ error: '文件名不能为空' });
    return;
  }
  if (!restoreCategories || !Array.isArray(restoreCategories) || restoreCategories.length === 0) {
    reply.status(400).send({ error: '请选择要恢复的分类' });
    return;
  }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- JSON-parsed backup data is dynamically typed across 8 category structures with deeply nested field access
  ...
  reply.send({ success: true, message: '备份数据恢复成功！相关功能配置已更新。' });
};
