    ...
    <div v-if="asset" class="h-full flex flex-col text-[var(--text-primary)] overflow-hidden">
      ...
      >
        ...
        <div class="relative z-10 w-full h-full bg-black/40 backdrop-blur-sm">
          <ModelViewer
            v-if="asset.type === 'GLB' || asset.type === 'GLTF'"
            :model-url="asset.url"
            auto-rotate
            class="w-full h-full"
          />
          <div v-else class="w-full h-full flex items-center justify-center">
            ...
            <div v-else class="text-slate-400 flex flex-col items-center">
              <component
                :is="getFormatIcon(asset.type)"
                class="w-16 h-16 mb-4 opacity-50 drop-shadow-md"
              />
        ...
        <!-- Gradient Overlay -->
        <div
          class="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-slate-950 to-transparent z-10 pointer-events-none"
        ></div>
      ...
      <div class="flex-1 overflow-y-auto p-8 space-y-8 relative">
        ...
        <div class="relative z-10">
          <h2
            class="text-2xl font-black mb-3 tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400"
          >
            {{ asset.title }}
          </h2>
          <p class="text-slate-400 text-sm leading-relaxed">
            {{ asset.description || t('assets.noDescription') }}
          </p>
        </div>

        <div class="grid grid-cols-2 gap-3 relative z-10">
          <div
            class="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5 backdrop-blur-sm hover:bg-white/10 transition-colors duration-300"
          >
            <div class="p-2 rounded-lg bg-accent/20 text-accent">
              <User class="w-4 h-4" />
            </div>
            <span class="text-sm font-medium text-slate-200 truncate">{{
              asset.user?.name || t('assets.unknownUser')
            }}</span>
          </div>
          <div
            class="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5 backdrop-blur-sm hover:bg-white/10 transition-colors duration-300"
          >
            ...
            <span class="text-sm font-medium text-slate-200 truncate">{{
              asset.category?.name || t('assets.uncategorized')
            }}</span>
          </div>
          <div
            class="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5 backdrop-blur-sm hover:bg-white/10 transition-colors duration-300"
          >
            ...
            <span class="text-sm font-medium text-slate-200">{{
              asset.size ? `${asset.size} MB` : t('assets.unknownSize')
            }}</span>
          </div>
          <div
            class="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5 backdrop-blur-sm hover:bg-white/10 transition-colors duration-300"
        ...
        <div v-if="parsedFormats.length > 0" class="relative z-10">
          <h3 class="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4">
            {{ t('assets.includedFormats') }}
          </h3>
      ...
      >
        ...
        >
          <div
            class="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out"
          ></div>
