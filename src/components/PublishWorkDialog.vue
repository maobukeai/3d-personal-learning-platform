...
<template>
  <Transition name="fade">
    <div v-if="modelValue" class="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-[80vw] max-h-[90vh] overflow-y-auto p-8 rounded-3xl shadow-2xl scrollbar-hide" style="background-color: var(--bg-card)">
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-xl font-bold" style="color: var(--text-primary)">发布/上传作品</h3>
          <button @click="closeDialog" style="color: var(--text-secondary)"><X class="w-5 h-5" /></button>
        </div>
            ...
            <div class="grid grid-cols-2 gap-6">
              <div class="space-y-5">
                <div>
                  ...
                  <p v-if="myApprovedAssets.length === 0" class="text-[10px] text-slate-400 mt-1 ml-1">暂无已审核通过的作品，请切换到“资产上传”或先上传资产</p>
                </div>

                <div>
                  <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">展示标题 *</label>
                  <input v-model="publishForm.title" type="text" class="w-full px-4 py-3 bg-slate-100 dark:bg-white/5 border-none rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all" placeholder="给作品起个响亮的名字" style="color: var(--text-primary)" />
                </div>

                <div>
                  <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">标签</label>
                  <input v-model="publishForm.tags" type="text" class="w-full px-4 py-3 bg-slate-100 dark:bg-white/5 border-none rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all" placeholder="用逗号分隔，如：Blender,3D渲染,角色建模" style="color: var(--text-primary)" />
        ...
        <template v-if="publishCategory === 'asset'">
          <div class="space-y-5">
            <div class="grid grid-cols-2 gap-6">
              <div class="space-y-5">
                <div>
                  <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">资产文件 *</label>
                  <div class="relative group h-32">
                    <input type="file" @change="handleAssetFileChange" accept=".glb,.gltf,.fbx,.obj,.stl,.dae,.3ds,.blend,.usdz,.abc,.zip" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
                    <div class="w-full h-full border-2 border-dashed rounded-2xl flex flex-col items-center justify-center gap-1 transition-all group-hover:border-indigo-500 group-hover:bg-indigo-500/5" style="border-color: var(--border-base)">
                      ...
                      <p class="text-xs font-medium" style="color: var(--text-secondary)">
                        {{ publishForm.assetFile ? publishForm.assetFile.name : '点击或拖拽上传资产文件' }}
                      </p>
                      <p class="text-[10px]" style="color: var(--text-muted)">支持 3D模型文件及压缩包</p>
                    </div>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">资源分类 *</label>
                    <el-select v-model="publishForm.assetCategory" placeholder="请选择分类" class="w-full custom-select-v2">
                  ...
                  <div>
                    ...
                    <div class="relative group h-11">
                      ...
                      <div class="w-full h-full border rounded-xl flex items-center justify-center gap-1 transition-all group-hover:border-indigo-500 bg-slate-100 dark:bg-white/5" style="border-color: var(--border-base)">
                ...
                <div>
                  <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">标签</label>
                  <input v-model="publishForm.tags" type="text" class="w-full px-4 py-3 bg-slate-100 dark:bg-white/5 border-none rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all" placeholder="用逗号分隔" style="color: var(--text-primary)" />
                </div>
              </div>
          ...
          <div class="flex gap-6">
            ...
            <div class="w-[40%] space-y-5 shrink-0">
              ...
              <div v-if="publishForm.isVideo">
                ...
                <input v-model="publishForm.videoUrl" type="text" class="w-full px-4 py-3 bg-slate-100 dark:bg-white/5 border-none rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all" placeholder="请输入外部视频平台链接" style="color: var(--text-primary)" />
              </div>
            ...
            <div class="w-[60%] min-w-0">
              <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">作品描述</label>
              <MarkdownEditor v-model="publishForm.description" placeholder="描述你的创作灵感、使用的技术和工具... 支持 Markdown 格式" height="500px" />
            </div>
          </div>
        </template>
