...
const getStatusBg = (status: string) => {
  switch (status) {
    case 'PENDING': return 'bg-amber-500/80'
    case 'APPROVED': return 'bg-emerald-500/80'
    case 'REJECTED': return 'bg-rose-500/80'
...
const getTypeLabel = (type: string) => {
  ...
  if (['GLB', 'GLTF', 'FBX', 'OBJ', 'STL', 'DAE', '3DS', 'BLEND', 'USDZ', 'ABC'].includes(t)) return '3D模型'
...
const filteredWorks = computed(() => {
  ...
  switch (sortBy.value) {
    ...
    case 'size_asc': result.sort((a, b) => (a.size || 0) - (b.size || 0)); break
  }
...
<template>
  <div class="flex-1 flex flex-col h-full overflow-hidden" style="background-color: var(--bg-app)">
    ...
    <!-- Stats Bar -->
    <div class="px-8 py-4 border-b shrink-0" style="background-color: var(--bg-card); border-color: var(--border-base)">
      <div class="flex items-center gap-6">
        <div class="flex items-center gap-2.5 px-4 py-2.5 rounded-xl" style="background-color: var(--bg-app)">
    ...
    <div class="border-b px-8 py-2 shrink-0 flex items-center justify-between" style="background-color: var(--bg-card); border-color: var(--border-base)">
      <div class="flex items-center gap-6">
        <button 
          v-for="tab in tabs" 
          :key="tab.label"
        ...
        >
          ...
          <span class="text-[10px] px-1.5 py-0.5 rounded-full" 
                :class="activeTab === tab.label ? 'bg-accent/10 text-accent' : 'bg-slate-100 dark:bg-white/5 text-slate-400'">
            {{ tab.count }}
          </span>
          <div v-if="activeTab === tab.label" class="absolute bottom-0 left-0 right-0 h-0.5 bg-accent rounded-full"></div>
        </button>
      </div>

      <div class="flex items-center gap-3">
        <div class="relative">
          <select v-model="sortBy" class="appearance-none pl-8 pr-8 py-1.5 border-none rounded-lg text-xs font-medium focus:outline-none focus:ring-2 focus:ring-accent/20 cursor-pointer" style="background-color: var(--bg-app); color: var(--text-primary)">
            <option v-for="opt in sortOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
          </select>
          <ArrowUpDown class="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2" style="color: var(--text-secondary)" />
        </div>
        <div class="flex items-center rounded-lg overflow-hidden border" style="border-color: var(--border-base)">
          <button @click="viewMode = 'grid'" class="p-1.5 transition-all" :class="viewMode === 'grid' ? 'bg-accent text-white' : ''" :style="viewMode !== 'grid' ? 'color: var(--text-secondary); background-color: var(--bg-app)' : ''">
            <LayoutGrid class="w-3.5 h-3.5" />
          </button>
          <button @click="viewMode = 'list'" class="p-1.5 transition-all" :class="viewMode === 'list' ? 'bg-accent text-white' : ''" :style="viewMode !== 'list' ? 'color: var(--text-secondary); background-color: var(--bg-app)' : ''">
    ...
    <div class="flex-1 overflow-y-auto p-8 scrollbar-hide">
      <div class="max-w-7xl mx-auto">
        <!-- Grid View -->
        <div v-if="viewMode === 'grid' && filteredWorks.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <div v-for="work in filteredWorks" :key="work.id" 
            ...
            <div class="aspect-video relative overflow-hidden bg-slate-100 dark:bg-white/5 flex items-center justify-center">
              ...
              <img v-else :src="getDefaultThumbnailUrl(work.type)" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
              ...
              <div class="absolute top-3 right-3">
                <span class="backdrop-blur px-2 py-1 rounded text-[10px] font-bold text-white/90 shadow-sm bg-black/40">
              ...
              <div class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                ...
                <button @click="openEditDialog(work)" class="p-2.5 rounded-xl hover:bg-blue-50 dark:hover:bg-blue-900/40 hover:text-blue-500 transition-all shadow-lg"
                        style="background-color: var(--bg-card); color: var(--text-primary)">
                  <Edit3 class="w-4 h-4" />
                </button>
            ...
            <div class="p-4 flex-1 flex flex-col">
              <div class="flex items-center justify-between mb-1">
                <h3 class="text-sm font-bold truncate pr-2" style="color: var(--text-primary)">{{ work.title }}</h3>
                <span v-if="work.category" class="text-[10px] font-bold text-accent px-1.5 py-0.5 bg-accent/10 rounded shrink-0">{{ work.category.name }}</span>
              </div>
              <p v-if="work.description" class="text-[11px] line-clamp-1 mb-2" style="color: var(--text-secondary)">{{ work.description }}</p>
              <p class="text-[10px] text-slate-400">{{ new Date(work.createdAt).toLocaleDateString() }}</p>
              
              <div class="mt-3 pt-3 border-t flex items-center justify-between" style="border-color: var(--border-base)">
        ...
        <div v-if="viewMode === 'list' && filteredWorks.length > 0" class="space-y-2">
          <div v-for="work in filteredWorks" :key="work.id"
            ...
            <div class="w-16 h-16 rounded-lg overflow-hidden shrink-0 bg-slate-100 dark:bg-white/5 flex items-center justify-center">
              <img v-if="work.thumbnail" :src="work.thumbnail" class="w-full h-full object-cover" />
              <img v-else :src="getDefaultThumbnailUrl(work.type)" class="w-full h-full object-cover" />
            </div>

            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-0.5">
                <h3 class="text-sm font-bold truncate" style="color: var(--text-primary)">{{ work.title }}</h3>
                <div class="backdrop-blur px-1.5 py-0.5 rounded text-[9px] font-bold text-white shrink-0"
                     :class="getStatusBg(work.status)">
                  {{ getStatusLabel(work.status) }}
                </div>
              </div>
              <p v-if="work.description" class="text-[11px] truncate" style="color: var(--text-secondary)">{{ work.description }}</p>
            </div>

            <div class="flex items-center gap-4 shrink-0 text-[11px]" style="color: var(--text-secondary)">
              <span v-if="work.category" class="px-2 py-0.5 rounded bg-accent/10 text-accent text-[10px] font-bold">{{ work.category.name }}</span>
              <span class="w-16 text-center">{{ getTypeLabel(work.type) }}</span>
              <span v-if="work.size" class="w-16 text-right">{{ work.size }} MB</span>
              <span class="w-20 text-right">{{ new Date(work.createdAt).toLocaleDateString() }}</span>
            </div>
            ...
            <div class="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <button @click.stop="handleDeleteWork(work)" class="p-2 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-900/40 text-rose-500 transition-all">
                <Trash2 class="w-4 h-4" />
              </button>
            </div>
          </div>
        ...
        <!-- Empty State -->
        <div v-if="filteredWorks.length === 0" class="h-80 flex flex-col items-center justify-center rounded-3xl border border-dashed" style="background-color: var(--bg-card); border-color: var(--border-base); color: var(--text-secondary)">
    ...
    <!-- Edit Dialog -->
    <Transition name="fade">
      <div v-if="isEditDialogOpen" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="isEditDialogOpen = false"></div>
        <div class="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto p-8 rounded-3xl shadow-2xl space-y-6" style="background-color: var(--bg-card)">
          <div class="flex items-center justify-between">
            <h3 class="text-xl font-bold" style="color: var(--text-primary)">编辑作品信息</h3>
            <button @click="isEditDialogOpen = false" style="color: var(--text-secondary)"><X class="w-5 h-5" /></button>
          </div>

          <div class="space-y-4">
            <div>
              <label class="block text-xs font-bold uppercase mb-2 ml-1" style="color: var(--text-secondary)">作品名称</label>
              <input v-model="editForm.title" type="text" class="w-full px-4 py-3 border rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-accent/20 transition-all" style="background-color: var(--bg-app); border-color: var(--border-base); color: var(--text-primary)" />
            </div>
            
            <div>
              <label class="block text-xs font-bold uppercase mb-2 ml-1" style="color: var(--text-secondary)">描述 (支持 Markdown)</label>
              <MarkdownEditor v-model="editForm.description" placeholder="添加作品描述... 支持 Markdown 格式" height="250px" />
            </div>

            <div>
              <label class="block text-xs font-bold uppercase mb-2 ml-1" style="color: var(--text-secondary)">资源分类</label>
              <el-select v-model="editForm.categoryId" placeholder="请选择分类" class="w-full custom-select-v2" clearable>
                <el-option v-for="cat in assetCategories" :key="cat.id" :label="cat.name" :value="cat.id" />
              </el-select>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3">
            <button @click="isEditDialogOpen = false" class="flex-1 py-3 border rounded-2xl text-sm font-bold transition-all" style="border-color: var(--border-base); color: var(--text-secondary)">取消</button>
            <button @click="handleSaveEdit" :disabled="isSaving" class="flex-1 py-3 bg-accent text-white rounded-2xl text-sm font-bold shadow-lg shadow-accent/20 transition-all flex items-center justify-center gap-2">
    ...
    <Transition name="fade">
      <div v-if="isPublishDialogOpen" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto p-8 rounded-3xl shadow-2xl space-y-6" style="background-color: var(--bg-card)">
          ...
          <div class="p-4 rounded-xl border flex items-center gap-3" style="background-color: var(--bg-app); border-color: var(--border-base)">
            <div class="w-12 h-12 rounded-lg overflow-hidden shrink-0 bg-slate-100 dark:bg-white/5 flex items-center justify-center">
              <Box class="w-6 h-6 text-accent" />
            </div>
            <div class="flex-1 min-w-0">
              <p class="text-sm font-bold truncate" style="color: var(--text-primary)">{{ publishForm.title }}</p>
              <p class="text-[10px]" style="color: var(--text-secondary)">此作品将发布到作品展示墙供大家浏览</p>
            </div>
          </div>

          <div class="space-y-4">
            <div>
              <label class="block text-xs font-bold uppercase mb-2 ml-1" style="color: var(--text-secondary)">展示标题</label>
              <input v-model="publishForm.title" type="text" class="w-full px-4 py-3 border rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all" style="background-color: var(--bg-app); border-color: var(--border-base); color: var(--text-primary)" placeholder="展示标题" />
            </div>

            <div>
              <label class="block text-xs font-bold uppercase mb-2 ml-1" style="color: var(--text-secondary)">展示描述 (支持 Markdown)</label>
              <MarkdownEditor v-model="publishForm.description" placeholder="描述你的创作灵感... 支持 Markdown 格式" height="250px" />
            </div>

            <div>
              <label class="block text-xs font-bold uppercase mb-2 ml-1" style="color: var(--text-secondary)">标签</label>
              <input v-model="publishForm.tags" type="text" class="w-full px-4 py-3 border rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all" style="background-color: var(--bg-app); border-color: var(--border-base); color: var(--text-primary)" placeholder="用逗号分隔，如：Blender,3D渲染,角色建模" />
            </div>
          </div>

          <div class="flex items-center gap-3">
            <button @click="isPublishDialogOpen = false" class="flex-1 py-3 border rounded-2xl text-sm font-bold transition-all" style="border-color: var(--border-base); color: var(--text-secondary)">取消</button>
            <button @click="handlePublishToShowcase" :disabled="isPublishing" class="flex-1 py-3 bg-indigo-600 text-white rounded-2xl text-sm font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all flex items-center justify-center gap-2">
      ...
      <div v-if="isPreviewOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-10">
        ...
        <div class="relative w-full max-w-5xl h-full rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row" style="background-color: var(--bg-card)">
          ...
          <div class="flex-1 relative group overflow-hidden" style="background-color: var(--bg-app)">
            <ModelViewer 
              v-if="selectedAsset?.type && ['GLB', 'GLTF', 'FBX', 'OBJ', 'STL'].includes(selectedAsset.type.toUpperCase())"
              :model-url="selectedAsset?.url" 
              :auto-rotate="isAutoRotating" 
              @metadata-loaded="handleMetadataLoaded"
            />
            <div v-else-if="selectedAsset?.thumbnail" class="w-full h-full flex items-center justify-center p-8">
              <img :src="selectedAsset.thumbnail" class="max-w-full max-h-full object-contain rounded-xl" />
            </div>
            <div v-else class="w-full h-full flex items-center justify-center">
              <img :src="getDefaultThumbnailUrl(selectedAsset?.type)" class="max-w-full max-h-full object-contain rounded-xl" />
            </div>
            ...
            <div class="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 backdrop-blur px-4 py-2 rounded-full shadow-lg border" 
              ...
              <button v-if="selectedAsset?.type && ['GLB', 'GLTF', 'FBX', 'OBJ', 'STL'].includes(selectedAsset.type.toUpperCase())" @click="isAutoRotating = !isAutoRotating" class="p-2 rounded-full transition-colors" :class="isAutoRotating ? 'text-accent bg-accent-subtle' : 'text-slate-400 hover:bg-slate-50'">
              ...
              <button @click="handleDeleteWork(selectedAsset)" class="p-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/40 rounded-full transition-colors">
                <Trash2 class="w-4 h-4" />
              </button>
            </div>
          </div>
          ...
          <!-- Side Info Panel -->
          <div class="w-full md:w-80 p-6 flex flex-col border-l overflow-y-auto" style="background-color: var(--bg-card); border-color: var(--border-base)">
            <div class="flex items-center justify-between mb-1">
              <h2 class="text-xl font-bold" style="color: var(--text-primary)">{{ selectedAsset?.title }}</h2>
            </div>
            <div class="flex items-center gap-2 mb-6">
              <span class="text-sm" style="color: var(--text-secondary)">{{ getTypeLabel(selectedAsset?.type) }}</span>
              <span class="px-2 py-0.5 rounded text-[10px] font-bold text-white" :class="getStatusBg(selectedAsset?.status)">
            ...
            <div class="space-y-4 flex-1">
              ...
              <div class="p-4 rounded-xl border" style="background-color: var(--bg-app); border-color: var(--border-base)">
                <p class="text-[10px] uppercase font-bold mb-3 tracking-wider" style="color: var(--text-secondary)">资产详情</p>
                <div class="grid grid-cols-2 gap-y-3 gap-x-4">
                  ...
                  <div v-if="selectedAsset?.materials">
                    ...
                    <p class="text-xs font-bold" style="color: var(--text-primary)">{{ selectedAsset.materials }} 个</p>
                  </div>
                  ...
                  <div v-if="selectedAsset?.dimensions">
                    ...
                    <p class="text-xs font-bold font-mono" style="color: var(--text-primary)">{{ selectedAsset.dimensions }}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p class="text-xs font-bold" style="color: var(--text-primary)">{{ new Date(selectedAsset?.createdAt).toLocaleDateString() }}</p>
                  </div>
                </div>
              </div>
            </div>
