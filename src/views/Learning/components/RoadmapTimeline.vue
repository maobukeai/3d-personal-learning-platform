...
import { CheckCircle2, Lock, Zap, ArrowRight, Sparkle } from 'lucide-vue-next';
...
const emit = defineEmits<{
  (e: 'update:activeStepId', stepId: string): void;
}>();
...
const isStepLocked = (_step: RoadmapStep, index: number | string) => {
  ...
  return prevStep && !props.isStepCompleted(prevStep.id);
};

const getStepStatus = (step: RoadmapStep, index: number | string) => {
  const idx = Number(index);
  if (props.isStepCompleted(step.id)) return 'completed';
  if (isStepLocked(step, idx)) return 'locked';
  if (idx === 0 || props.isStepCompleted(props.steps[idx - 1]?.id)) return 'current';
  return 'upcoming';
};

const selectStep = (stepId: string) => {
  emit('update:activeStepId', stepId);
};
</script>
  ...
  <div class="roadmap-left-panel col-span-5 md:col-span-7 lg:col-span-8 space-y-3 sm:space-y-4">
    <div class="relative pl-[23px] sm:pl-10 space-y-3 sm:space-y-5 py-1 sm:py-2">
      ...
      >
        ...
        >
          <div class="mobile-row flex items-start justify-between gap-1.5 sm:gap-4">
