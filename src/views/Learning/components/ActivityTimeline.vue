...
<template>
  <div class="flex flex-col xl:flex-row gap-5 items-start w-full animate-fade-in">
    ...
    >
      ...
      >
        ...
        >
          ...
          <p
            class="text-[11px] sm:text-xs md:text-sm text-[var(--text-secondary)] mt-1 leading-relaxed line-clamp-2"
          >
        ...
        <div
          class="flex items-center justify-between mt-1 pt-2.5 border-t border-[var(--border-base)]"
        >
          <div
            class="flex items-center gap-5 text-[10px] md:text-xs font-bold text-[var(--text-muted)]"
          >
            <!-- Likes -->
            <button
              type="button"
          ...
          <div v-else class="flex gap-1 items-center">
            ...
            <button
              type="button"
              class="p-1 text-red-500 hover:text-red-600 transition-all cursor-pointer"
              @click.stop="emit('delete', note)"
            >
              <Trash2 class="w-3 h-3" />
            </button>
        ...
        <Transition name="slide-fade">
          <div
            v-if="activeCommentsNoteId === note.id"
            class="mt-2.5 pt-2.5 border-t border-[var(--border-base)] flex flex-col gap-2.5"
          >
            <!-- Loader -->
            ...
            <!-- Empty state -->
            <div
              v-else-if="!noteCommentsMap[note.id] || !noteCommentsMap[note.id].length"
              class="text-center py-2 text-[10px] text-[var(--text-muted)] font-bold"
            >
              暂无评论，留下你的一笔启发吧~
            </div>
            ...
            <div v-else class="flex flex-col gap-2 max-h-52 overflow-y-auto custom-scrollbar pr-1">
              <div
                v-for="c in noteCommentsMap[note.id] || []"
                :key="c.id"
              ...
              >
                ...
                />
                <div
                  class="flex-1 bg-slate-50 dark:bg-white/[0.02] border border-[var(--border-base)] rounded-xl p-2 relative group/comment"
                >
                  ...
                  <p
                    class="text-[10px] md:text-[11px] text-[var(--text-secondary)] mt-1 whitespace-pre-line leading-relaxed"
                  >
                    {{ c.content }}
                  </p>
    ...
    <aside
      class="hidden xl:flex flex-col w-80 shrink-0 bg-[var(--bg-card)] border border-[var(--border-base)] rounded-2xl p-4 space-y-4 shadow-sm sticky top-20"
    >
      ...
      <div class="pt-0.5">
        <div
          class="bg-gradient-to-br from-accent/5 to-purple-500/5 dark:from-accent/10 dark:to-purple-500/10 border border-[var(--border-base)] p-3.5 rounded-xl relative overflow-hidden backdrop-blur-sm group/quote"
        >
          <div
            class="absolute -right-3 -bottom-3 text-accent/10 pointer-events-none transition-transform group-hover/quote:scale-110 duration-500"
          >
      ...
      <div v-if="topContributors.length" class="space-y-2 pt-1.5">
        ...
        <div class="flex flex-col gap-1.5">
          <div
            v-for="(item, idx) in topContributors"
            :key="item.user.id"
            class="flex items-center justify-between gap-2 p-1.5 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 transition-all"
          >
      ...
      <div class="space-y-2 pt-1.5">
        ...
        <div class="flex flex-wrap gap-1">
          ...
          <span v-if="!props.tags.length" class="text-[10px] text-[var(--text-muted)]"
            >暂无话题</span
          >
        </div>
      </div>
      ...
      <div class="space-y-2 pt-1.5">
        ...
        <div class="flex flex-col gap-1">
          <button
            ...
            @click="emit('filter-category', props.filterCategory === c ? '' : c)"
          >
