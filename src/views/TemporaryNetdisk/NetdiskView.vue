...
const handleUpload = async (fileList: FileList | null) => {
  ...
  try {
    ...
    if (activeMultipart) {
      await api
        .post('/api/temporary-netdisk/multipart/abort', {
...
<template>
  <div class="w-full px-4 sm:px-8 py-5 space-y-5 animate-in fade-in duration-500">
    ...
    <div
      v-if="!loading && !hasConfig"
      class="flex flex-col items-center justify-center min-h-[400px] text-center p-8 bg-[var(--bg-card)] border border-[var(--border-base)] rounded-3xl space-y-4"
    >
      <div
        class="w-16 h-16 rounded-full bg-rose-500/10 text-rose-500 flex items-center justify-center mx-auto shadow-sm animate-bounce"
      >
        <Lock class="w-8 h-8" />
      </div>
      ...
      <p class="text-xs text-[var(--text-muted)] max-w-[420px] leading-relaxed">
        系统管理员尚未配置或启用临时网盘专用的云端对象存储 (Cloudflare
        R2)。为了保证文件传输安全性，临时网盘在此环境下处于停用状态。
      </p>
      <div class="pt-2">
    ...
    <template v-else>
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-xs text-[var(--text-muted)] mt-1 leading-relaxed max-w-[680px]">
            ...
            <strong class="text-rose-500"
              >系统已设定每天 {{ cleanupTime }} 自动清空所有文件与分享</strong
            >，请及时下载。
          </p>
        </div>
        ...
        >
          ...
          <div
            class="flex items-center justify-between text-[10px] text-[var(--text-muted)] font-mono"
          >
      ...
      >
        ...
        <div class="text-[11px]">
          ...
          <p class="mt-0.5 leading-relaxed opacity-95">
            此网盘属于临时中转站。每日系统在触发清理点（大约
            {{ cleanupTime }}）时，会永久擦除所有文件与数据库分享记录。请务必及时下载。
          </p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
          ...
          >
            ...
            <div v-if="isUploading" class="space-y-2.5 w-full px-2" @click.stop>
              ...
              <p
                class="text-xs font-bold text-[var(--text-primary)] truncate"
                :title="uploadStatusText"
              >
                {{ uploadStatusText || '正在上传文件...' }}
              </p>
              <div
                class="w-full bg-[var(--border-base)] rounded-full h-2 overflow-hidden shadow-inner"
              >
                <div
              ...
              <div
                class="flex items-center justify-between text-[10px] text-[var(--text-muted)] font-mono px-0.5"
              >
                <span
                  >{{ formatSize(uploadLoadedBytes) }} / {{ formatSize(uploadTotalBytes) }}</span
                >
                <span
                  v-if="uploadSpeedStr && uploadProgress < 99"
                  class="text-indigo-500 font-bold font-sans"
                  >{{ uploadSpeedStr }}</span
                >
            ...
            <template v-else>
              ...
              <p
                class="text-[10px] text-[var(--text-muted)] mt-1 leading-relaxed max-w-[200px] mx-auto"
              >
                拖拽文件到此，或点击此处浏览上传
              </p>
            </template>
        ...
        <div class="lg:col-span-2 space-y-4">
          ...
          <div
            v-if="loading"
            class="flex items-center justify-center py-20 bg-[var(--bg-card)] rounded-3xl border border-[var(--border-base)]"
          >
          ...
          <div
            ...
            class="p-12 text-center bg-[var(--bg-card)] rounded-3xl border border-[var(--border-base)] text-[var(--text-muted)] text-xs"
          >
            ...
            >
              <div class="flex items-center gap-3.5 min-w-0">
                ...
                <div class="min-w-0 text-xs">
                  ...
                  <div
                    class="flex items-center gap-2.5 text-[var(--text-muted)] mt-1 font-mono text-[10px]"
                  >
                    <span>{{ formatSize(file.size) }}</span>
                    <span class="w-1 h-1 rounded-full bg-[var(--border-base)]"></span>
                    <span>{{ formatDate(file.createdAt) }}</span>
                    <span
                      v-if="file.shares.length > 0"
                      class="w-1 h-1 rounded-full bg-[var(--border-base)]"
                    ></span>
      ...
      <!-- Share Link Dialog -->
      <FormDialog v-model:visible="shareDialogVisible" title="创建分享链接" size="md">
        <div v-if="selectedFile" class="space-y-5 py-2 text-xs">
          <div
            class="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 dark:bg-white/5 border border-[var(--border-base)]"
          >
            <div
              :class="[
                'w-9 h-9 rounded-xl flex items-center justify-center shrink-0',
                getFileIconColor(selectedFile.name),
              ]"
            >
              <component
                :is="getFileIcon(selectedFile.mimeType, selectedFile.name)"
                class="w-4 h-4"
              />
            </div>
            <div class="min-w-0">
          ...
          <template v-if="!shareCreated">
            ...
            <div class="space-y-3">
              <div class="flex items-center gap-2">
                ...
                <label
                  class="font-bold text-[var(--text-secondary)] cursor-pointer"
            ...
            <div class="space-y-2">
              ...
              <Select v-model="expiresDays" class="w-full custom-dialog-input">
                <SelectOption value="1" label="1天有效" />
                <SelectOption value="3" label="3天有效" />
                <SelectOption value="7" label="7天有效" />
                <SelectOption value="30" label="30天有效" />
                <SelectOption value="0" label="长期有效 (直至每日 auto-clean 清理点)" />
              </Select>
              <p class="text-[10px] text-amber-500 flex items-center gap-1 mt-1">
                <Clock class="w-3 h-3" />
                注意：链接最长仅存活至每日
                {{ cleanupTime }} 擦除时间，设置天数仅做在此之前的防刷缓存。
              </p>
            </div>

            <!-- Submit -->
            <div class="flex justify-end gap-2 pt-2">
              <Button variant="outline" @click="shareDialogVisible = false">取消</Button>
              <Button variant="primary" :disabled="isCreatingShare" @click="generateShare">
          ...
          <template v-else>
            ...
            <div class="space-y-4">
              <div
                class="p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/10 text-indigo-600 dark:text-indigo-400"
              >
              ...
              <div
                class="space-y-3 p-4 bg-slate-50 dark:bg-white/5 border border-[var(--border-base)] rounded-2xl font-mono text-[11px] leading-relaxed"
              >
                <p>文件名称：{{ selectedFile.name }}</p>
                <p>文件大小：{{ formatSize(selectedFile.size) }}</p>
                <p>
                  下载链接：<span class="text-indigo-500 break-all select-all">{{
                    generatedShareUrl
                  }}</span>
                </p>
                <p v-if="usePassword && sharePassword">
                  提取密码：<strong
                    class="text-[var(--text-primary)] font-bold text-xs select-all"
              ...
              <!-- Actions -->
              <div class="flex justify-end gap-2 pt-2">
                <Button variant="outline" @click="shareDialogVisible = false">完成</Button>
                <Button variant="primary" @click="copyShareLink">
