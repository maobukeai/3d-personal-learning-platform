...
const plugin = ref<any | null>(null);
const customText = ref<string | null>(null);
const loading = ref(true);
const errorMsg = ref('');
const isExpired = ref(false);

const loadSharedPlugin = async () => {
  loading.value = true;
  errorMsg.value = '';
  isExpired.value = false;
  try {
    const res = await api.get(`/api/plugins/share/${shareId}?t=${Date.now()}`);
    plugin.value = res.data.plugin;
    customText.value = res.data.customText;

    document.title = `${res.data.plugin.title} | ${systemStore.settings.PLATFORM_NAME || '3D 学习平台'}`;
  } catch (error: any) {
    if (error.response?.status === 410) {
      isExpired.value = true;
      errorMsg.value = '该分享链接已过期失效';
    } else {
      errorMsg.value = error.response?.data?.error || '无法加载分享的插件，链接可能不存在或已被取消';
    }
  ...
  >
    ...
    >
      <div class="flex items-center gap-2 cursor-pointer select-none" @click="goHome">
        ...
        <span
          class="text-xs font-black bg-clip-text text-transparent bg-gradient-to-r from-amber-600 to-orange-600 dark:from-amber-400 dark:to-orange-400"
        >
    ...
    <div
      v-if="customText"
      class="bg-gradient-to-r from-amber-500/[0.04] to-orange-500/[0.04] border-b border-amber-500/10 px-6 py-2.5 text-center text-xs font-bold text-amber-600 dark:text-amber-400 flex items-center justify-center gap-2 shrink-0"
    >
    ...
    >
      ...
      >
        ...
        <p class="text-xs text-[var(--text-secondary)] max-w-sm mb-6 leading-relaxed">
          {{ errorMsg || '该插件共享可能已到期，或已被作者停止公共访问。' }}
        </p>
      ...
      <template v-else-if="plugin">
        <GlassCard padding="none" class="shadow-xl p-3 sm:p-6">
          <PluginDetailModal :plugin="plugin" inline />
        </GlassCard>
      </template>
    </main>
  </div>
