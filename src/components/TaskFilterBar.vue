...
const groupBy = defineModel<'status' | 'priority' | 'assignee' | 'dueDate'>('groupBy', { required: true });
const sortBy = defineModel<'natural' | 'createdAt_asc' | 'createdAt_desc'>('sortBy', { required: true });
...
const subtaskDisplay = defineModel<'collapse' | 'expand' | 'separate'>('subtaskDisplay', { required: true });
...
const priorityOptions = computed(() => [
  ...
  { id: 'LOW', label: t('tasks.low'), textColor: 'text-slate-455', icon: ArrowDown },
]);
...
<template>
  ...
  >
    ...
    <div class="flex items-center gap-2 sm:gap-2.5 min-w-0">
      ...
      <button
        ...
        :class="
          onlyMyTasks
            ...
            : 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-350 hover:bg-slate-200/50 dark:hover:bg-white/10'
        "
      ...
      <button
        ...
        :class="
          hideCompleted
            ? 'bg-accent/15 border-accent/25 text-accent shadow-sm'
            : 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-355 hover:bg-slate-200/50 dark:hover:bg-white/10'
        "
        @click="hideCompleted = !hideCompleted"
      >
        <EyeOff v-if="hideCompleted" class="w-3.5 h-3.5 text-accent" />
        <Eye v-else class="w-3.5 h-3.5 text-slate-450" />
        <span>隐藏已完成</span>
      </button>
      ...
      <!-- Time Dropdown -->
      <el-dropdown trigger="click" @command="(val: string) => dateFilter = val" popper-class="glass-popover">
        <button
          type="button"
        ...
        >
          <Calendar class="w-3.5 h-3.5 shrink-0" :class="dateFilter !== 'all' ? 'text-accent' : 'text-slate-450'" />
      ...
      <!-- Status Dropdown -->
      <el-dropdown trigger="click" @command="(val: string) => statusFilter = val" popper-class="glass-popover">
        <button
          type="button"
          class="px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 border"
          :class="statusFilter !== 'all'
            ...
            : 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200/50 dark:hover:bg-white/10'"
        >
          <div class="w-1.5 h-1.5 rounded-full shrink-0" :class="statusFilter !== 'all' ? 'bg-accent' : 'bg-slate-450'"></div>
      ...
      <!-- Priority Dropdown -->
      <el-dropdown trigger="click" @command="(val: string) => priorityFilter = val" popper-class="glass-popover">
        <button
          type="button"
          class="px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 border"
          :class="priorityFilter !== 'all'
            ...
            : 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200/50 dark:hover:bg-white/10'"
        >
          <Flame class="w-3.5 h-3.5 shrink-0" :class="priorityFilter !== 'all' ? 'text-accent' : 'text-slate-455'" />
          ...
          <el-dropdown-menu>
            ...
            >
              <div class="flex items-center gap-1.5">
                <component :is="opt.icon" v-if="opt.icon" class="w-3.5 h-3.5" :class="opt.textColor" />
                <span :class="opt.textColor">{{ opt.label }}</span>
              </div>
            </el-dropdown-item>
          </el-dropdown-menu>
      ...
      <!-- Assignee Dropdown -->
      <el-dropdown trigger="click" @command="(val: string) => assigneeFilter = val" popper-class="glass-popover">
        <button
          type="button"
          class="px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 border"
          :class="assigneeFilter !== 'all'
            ? 'bg-accent/15 border-accent/25 text-accent shadow-sm'
            : 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200/50 dark:hover:bg-white/10'"
        >
          <User class="w-3.5 h-3.5 shrink-0" :class="assigneeFilter !== 'all' ? 'text-accent' : 'text-slate-455'" />
          <span>成员: {{ assigneeFilter === 'all' ? '全部' : (assigneeFilter === 'unassigned' ? '未指派' : teamMembers.find(m => m.id === assigneeFilter)?.name || '成员') }}</span>
          <ChevronDown class="w-3 h-3 opacity-60" />
        </button>
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item command="all" :class="{ 'is-active text-accent font-bold': assigneeFilter === 'all' }">
              全部成员
            </el-dropdown-item>
            <el-dropdown-item command="unassigned" :class="{ 'is-active text-accent font-bold': assigneeFilter === 'unassigned' }">
              未指派
            </el-dropdown-item>
            <el-dropdown-item
            ...
            >
              <div class="flex items-center gap-2">
                <img v-if="m.avatarUrl" :src="m.avatarUrl" class="w-4 h-4 rounded-full object-cover" />
                <span>{{ m.name }}</span>
              </div>
            </el-dropdown-item>
          </el-dropdown-menu>
      ...
      <!-- Tag Dropdown -->
      <el-dropdown trigger="click" @command="(val: string) => tagFilter = val" popper-class="glass-popover">
        <button
          type="button"
          class="px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 border"
          :class="tagFilter !== 'all'
            ...
            : 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200/50 dark:hover:bg-white/10'"
        >
          <Layers class="w-3.5 h-3.5 shrink-0" :class="tagFilter !== 'all' ? 'text-accent' : 'text-slate-455'" />
          <span>标签: {{ tagFilter === 'all' ? '全部' : tagFilter }}</span>
          <ChevronDown class="w-3 h-3 opacity-60" />
        </button>
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item command="all" :class="{ 'is-active text-accent font-bold': tagFilter === 'all' }">
              全部标签
            </el-dropdown-item>
            <el-dropdown-item
            ...
            <el-dropdown-item v-if="allTags.length === 0" disabled>
              暂无标签
            </el-dropdown-item>
          </el-dropdown-menu>
        </template>
      ...
      <el-dropdown trigger="click" @command="(val: any) => subtaskDisplay = val" popper-class="glass-popover">
        <button
          type="button"
          class="px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 border"
          :class="subtaskDisplay !== 'collapse'
            ? 'bg-accent/15 border-accent/25 text-accent shadow-sm'
            : 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200/50 dark:hover:bg-white/10'"
        >
          <CheckSquare class="w-3.5 h-3.5 shrink-0" :class="subtaskDisplay !== 'collapse' ? 'text-accent' : 'text-slate-455'" />
          <span>子任务: {{ subtaskDisplay === 'collapse' ? '折叠' : (subtaskDisplay === 'expand' ? '展开' : '独立任务') }}</span>
          <ChevronDown class="w-3 h-3 opacity-60" />
        </button>
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item command="collapse" :class="{ 'is-active text-accent font-bold': subtaskDisplay === 'collapse' }">
              折叠所有子任务
            </el-dropdown-item>
            <el-dropdown-item command="expand" :class="{ 'is-active text-accent font-bold': subtaskDisplay === 'expand' }">
              展开所有子任务
            </el-dropdown-item>
            <el-dropdown-item command="separate" :class="{ 'is-active text-accent font-bold': subtaskDisplay === 'separate' }">
              作为独立任务显示
            </el-dropdown-item>
          </el-dropdown-menu>
        </template>
    ...
    <div class="flex items-center gap-2 sm:gap-2.5 shrink-0 ml-auto">
      <!-- Group By Dropdown -->
      <el-dropdown trigger="click" @command="(val: any) => groupBy = val" popper-class="glass-popover">
        <button
          type="button"
          class="px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-bold transition-all flex items-center gap-1 cursor-pointer border"
          :class="groupBy !== 'status'
            ? 'bg-accent/15 border-accent/25 text-accent shadow-sm'
            : 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200/50 dark:hover:bg-white/10'"
        >
          <Layers class="w-3.5 h-3.5 shrink-0" :class="groupBy !== 'status' ? 'text-accent' : 'text-slate-450'" />
      ...
      <!-- Sort By Dropdown -->
      <el-dropdown trigger="click" @command="(val: any) => sortBy = val" popper-class="glass-popover">
        <button
          type="button"
          class="px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-bold transition-all flex items-center gap-1 cursor-pointer border"
          :class="sortBy !== 'natural'
            ? 'bg-accent/15 border-accent/25 text-accent shadow-sm'
            : 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200/50 dark:hover:bg-white/10'"
        >
          <SortAsc class="w-3.5 h-3.5 shrink-0" :class="sortBy !== 'natural' ? 'text-accent' : 'text-slate-450'" />
      ...
      >
        <ArrowUpDown class="w-3.5 h-3.5 text-slate-450" :class="{ 'transform rotate-180 text-accent': sortOrder === 'desc' }" />
      </button>
      ...
      <button
        ...
        type="button"
        class="px-2 py-1 rounded-lg text-[10px] sm:text-xs font-bold transition-all flex items-center gap-1 border border-dashed border-rose-300 dark:border-rose-700/60 text-rose-500 hover:bg-rose-500/10 hover:border-rose-450 cursor-pointer shrink-0"
