...
const defaultHealthVariant = (label: string): 'primary' | 'success' | 'warning' | 'danger' | 'info' | 'blender' => {
  if (['正常', '稳定', '已同步', '已连接', '健康', '已清空', '无暂停', '常态', '队列健康'].includes(label)) {
    return 'success';
  }
...
const getDotBgClass = (card: StatItem) => {
  ...
  if (colorStr.includes('rose') || colorStr.includes('danger') || colorStr.includes('red')) return 'bg-rose-500';
  if (colorStr.includes('amber') || colorStr.includes('warning')) return 'bg-amber-500';
  if (colorStr.includes('purple')) return 'bg-purple-500';
  if (colorStr.includes('sky') || colorStr.includes('indigo') || colorStr.includes('blue') || colorStr.includes('primary')) return 'bg-sky-500';
  return 'bg-slate-400';
};

const isSuccessStatus = (label: string): boolean => {
  return ['正常', '稳定', '已同步', '已连接', '健康', '已清空', '无暂停', '常态', '队列健康', '无异常', '空闲', '运行中', '已配置'].includes(label);
};
</script>
...
<template>
  ...
  <Card padding="none" class="!py-1.5 !px-3 sm:!px-4 border-base bg-card shadow-sm">
    <div class="flex flex-col lg:grid lg:grid-cols-[1fr_auto_1fr] items-center gap-2.5 w-full min-w-0">
      <!-- Left: Title & Minimalist Inline Indicators -->
      <div class="flex items-center gap-2.5 min-w-0 w-full justify-start z-10">
        <div class="flex items-center gap-1.5 shrink-0">
          <span v-if="subtitle" class="text-[11px] font-medium text-[var(--text-muted)]">{{ subtitle }}</span>
        ...
        <div v-if="cards && cards.length > 0" class="h-3.5 w-px bg-[var(--border-base)] hidden md:block shrink-0"></div>

        <!-- Minimalist Inline Stats -->
        <div v-if="cards && cards.length > 0" class="hidden md:flex items-center gap-2.5 text-[11px] overflow-x-auto scrollbar-hide min-w-0">
          <span
            v-for="card in cards"
            :key="card.label"
          ...
          >
            <span
              ...
              :class="getDotBgClass(card)"
            ></span>
            <span class="text-[var(--text-muted)] whitespace-nowrap">{{ card.label }}</span>
            <span
              class="font-bold text-[var(--text-primary)]"
              :class="(card.label === '待审核资源' || card.label === '待处理') && Number(card.value) > 0 ? 'text-rose-500 font-extrabold' : ''"
            >
              {{ card.value }}
            </span>
            <Badge
