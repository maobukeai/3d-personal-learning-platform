...
<template>
  <div class="manual-resource-detail h-full overflow-y-auto p-4 md:p-6 w-full max-w-[1400px] mx-auto scrollbar-hide">
    <!-- Back breadcrumb -->
...
type="button"
    ...
    <!-- Page Loading -->
    <div v-if="isLoading" class="flex flex-col items-center justify-center py-32 bg-white dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800/50">
      <Loader2 class="w-8 h-8 animate-spin text-cyan-500 mb-3" />
      <span class="text-sm font-medium text-slate-400">正在为您加载资源详情...</span>
    </div>

    <!-- Error Handling -->
    <div v-else-if="error" class="text-center py-20 bg-white dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800/50 p-6">
...
type="button"
    ...
    <template v-else-if="resource">
      ...
      <div v-if="!authStore.isAuthenticated" class="mb-6 p-4.5 rounded-2xl bg-cyan-500/5 dark:bg-cyan-500/5 border border-cyan-500/10 dark:border-cyan-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm backdrop-blur-sm">
        <div class="flex items-center gap-3">
          <AlertCircle class="w-5 h-5 text-cyan-500 shrink-0" />
          <div class="text-xs sm:text-sm text-cyan-800 dark:text-cyan-300 font-bold">
...
type="button" 
      ...
      <div v-else-if="resource.hasAccess === false" class="mb-6 p-4.5 rounded-2xl bg-amber-500/5 dark:bg-amber-500/5 border border-amber-500/10 dark:border-amber-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm backdrop-blur-sm">
        <div class="flex items-center gap-3">
          <Lock class="w-5 h-5 text-amber-500 shrink-0 animate-pulse" />
          <div>
            <div class="text-xs sm:text-sm text-amber-800 dark:text-amber-300 font-bold">
              会员获取权限不足
            </div>
...
type="button" 
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="lg:col-span-2 space-y-6">
          <div class="bg-white dark:bg-slate-900/40 rounded-2xl border border-slate-200 dark:border-slate-800/50 overflow-hidden shadow-sm">
            <!-- Thumbnail Cover -->
            <div v-if="resource.thumbnailUrl" class="w-full aspect-[16/9] max-h-[500px] bg-slate-100 dark:bg-slate-800 overflow-hidden relative">
              <img
            ...
            <div class="p-6">
              <div class="flex items-start gap-3 mb-4">
                <div class="flex-1">
                  <div class="flex items-center gap-2 mb-2">
                    <span v-if="resource.category" class="px-2.5 py-0.5 text-[10px] rounded-lg bg-cyan-50 dark:bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 font-black tracking-wide">
                      {{ resource.category.name }}
                    </span>
                    <span class="px-2.5 py-0.5 text-[10px] rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wide">
              ...
              <!-- Extra Meta Info -->
              <div class="flex flex-wrap items-center gap-4 text-xs text-slate-400 dark:text-slate-500 mb-6 border-b border-slate-100 dark:border-slate-800/80 pb-5">
                <span class="flex items-center gap-1.5 font-semibold text-slate-500">
                  <BookOpen class="w-4 h-4 text-cyan-500" />
                  手动发布
                </span>
                ...
                <span class="flex items-center gap-1.5">
                  <Eye class="w-4 h-4 text-slate-300" />
                  {{ resource.viewCount }} 次浏览
                </span>
              </div>

              <!-- Tag badges -->
              <div v-if="parseTags(resource.tags).length > 0" class="flex flex-wrap gap-2 mb-6">
                <span
                  v-for="tag in parseTags(resource.tags)"
                  :key="tag"
                  class="inline-flex items-center gap-1 px-3 py-1 text-xs rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200/40 dark:border-slate-800/50 text-slate-600 dark:text-slate-400 font-medium"
                >
              ...
              <div class="block lg:hidden mb-6">
                <div class="bg-slate-50 dark:bg-slate-900/60 border border-slate-150 dark:border-slate-800 rounded-2xl p-5">
                  <h3 class="flex items-center gap-2 text-sm font-black text-slate-800 dark:text-slate-200 mb-4 border-b border-slate-100 dark:border-slate-800 pb-3 uppercase tracking-wider">
                      ...
                      >
                        <div class="flex items-center justify-between">
                          <div class="flex items-center gap-2">
                            <div class="w-2.5 h-2.5 rounded-full" :class="getLinkTypeColor(link.type)"></div>
                            <span class="text-xs font-bold text-slate-700 dark:text-slate-200">{{ link.name }}</span>
                          </div>
                          <span class="text-[9px] uppercase font-black px-1.5 py-0.5 rounded bg-slate-50 dark:bg-slate-800 text-slate-400">
                            {{ link.type }}
                          </span>
                        </div>
...
type="button"
                ...
                <!-- Like Box Mobile -->
                <div class="mt-4 bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 flex items-center justify-between">
...
type="button"
                    ...
                    @click="toggleLike"
                  >
                    <Heart class="w-4 h-4" :class="{ 'fill-current animate-pulse text-rose-500': resource.hasLiked }" />
              ...
              <div v-else-if="resource.description" class="prose prose-sm dark:prose-invert max-w-none pt-4">
                <h3 class="text-sm font-bold text-slate-800 dark:text-slate-200 mb-2 border-l-2 border-cyan-500 pl-2">资源简介</h3>
          ...
          <!-- Comments Section -->
          <div class="bg-white dark:bg-slate-900/40 rounded-2xl border border-slate-200 dark:border-slate-800/50 p-6 space-y-6 shadow-sm">
            ...
            <div class="flex gap-3">
              <div class="w-8 h-8 rounded-full bg-gradient-to-r from-cyan-500 to-emerald-500 flex items-center justify-center text-white text-xs font-black shrink-0 shadow-md">
...
type="button"
            ...
            <!-- Comments List -->
            <div v-if="!resource.comments || resource.comments.length === 0" class="text-center py-12 text-slate-450 text-xs">
...
v-if="comment.user?.avatarUrl"
                    ...
                    loading="lazy"
                  />
                  <div v-else class="w-full h-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-500 text-[10px] font-black uppercase">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between mb-1">
                    ...
                    <span class="text-[10px] text-slate-400 dark:text-slate-500">{{ formatDate(comment.createdAt) }}</span>
                  </div>
                  <p class="text-xs sm:text-sm text-slate-650 dark:text-slate-350 whitespace-pre-wrap break-words leading-relaxed">
                    {{ comment.content }}
                  </p>
                </div>
              </div>
        ...
        <div class="hidden lg:block lg:sticky lg:top-6 space-y-6">
          <div class="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/50 rounded-2xl p-5 shadow-sm">
            <h3 class="flex items-center gap-2 text-sm font-black text-slate-800 dark:text-slate-200 mb-4 border-b border-slate-100 dark:border-slate-800/80 pb-3 uppercase tracking-wider">
              <Link2 class="w-4 h-4 text-cyan-500" />
              安全网盘通道
            </h3>
              ...
              <div v-else class="space-y-3">
                ...
                >
                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                      <div class="w-2.5 h-2.5 rounded-full" :class="getLinkTypeColor(link.type)"></div>
                      <span class="text-xs font-bold text-slate-700 dark:text-slate-250">{{ link.name }}</span>
                    </div>
                    <span class="text-[9px] uppercase font-black px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-400">
                      {{ link.type }}
                    </span>
                  </div>
...
type="button"
          ...
          <!-- Likes Sticky card -->
          <div class="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/50 rounded-2xl p-5 shadow-sm flex items-center justify-between">
...
type="button"
    ...
    <Teleport to="body">
      ...
      >
        <div class="glass-dialog rounded-2xl w-full max-w-md mx-4 shadow-2xl overflow-hidden border border-white/10 bg-slate-900 text-white animate-in fade-in zoom-in-95 duration-200">
          <!-- Header -->
          <div class="flex items-center justify-between p-5 border-b border-white/5">
            <h2 class="text-sm font-black flex items-center gap-2 text-cyan-400 uppercase tracking-wider">
              <Link2 class="w-5 h-5 text-cyan-400" />
              已为您提取网盘下载地址
            </h2>
            <button
type="button"
          ...
          <div class="p-5 space-y-5 bg-slate-950/40">
            ...
            <div class="p-4 bg-white/5 rounded-xl border border-white/5 flex items-center justify-between">
              <div class="flex items-center gap-3">
                <div class="w-2.5 h-2.5 rounded-full animate-ping shrink-0" :class="getLinkTypeColor(activeLink.type)"></div>
                <div>
                  <div class="text-xs font-bold text-slate-100">{{ activeLink.name }}</div>
                  <div class="text-[10px] text-slate-400 uppercase mt-0.5 font-bold">
            ...
            <div class="space-y-1.5">
              <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wide">提取地址 / 下载链接</label>
...
type="button"
            ...
            <p class="text-[10px] text-slate-400 leading-normal bg-slate-900 p-3 rounded-lg border border-white/5">
              💡 **温馨提示**: 网盘服务由第三方提供，如遇到“链接已失效”或“链接不存在”，请联系系统管理员进行手动补档。
            </p>
          </div>
