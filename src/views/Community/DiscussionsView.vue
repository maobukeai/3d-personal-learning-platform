...
import { Search, MessageSquare, Edit3, X, Plus, Heart, Eye, Pin, Trash2, ChevronDown, ChevronUp, Send, Tag, Flame, Clock } from 'lucide-vue-next'
...
const handleCreateDiscussion = async () => {
  ...
  if (postForm.value.tags) {
    const tagsArray = postForm.value.tags.split(',').map(t => t.trim()).filter(Boolean)
...
const handleReplyComment = async (parentId: string) => {
  ...
  try {
    ...
    if (parentComment) {
      ...
      parentComment._count = { ...parentComment._count, replies: (parentComment._count?.replies || 0) + 1 }
    }
    replyContent.value = ''
    replyingTo.value = null
...
const deleteComment = async (comment: any, parentComment?: any) => {
  try {
    ...
    if (parentComment) {
      ...
      parentComment._count = { ...parentComment._count, replies: (parentComment._count?.replies || 1) - 1 }
    } else {
      selectedDiscussion.value.comments = selectedDiscussion.value.comments.filter((c: any) => c.id !== comment.id)
    }
    ElMessage.success('已删除')
    fetchDiscussions()
  } catch (error) {
...
<template>
  <div class="flex flex-col h-full" style="background-color: var(--bg-app)">
    <!-- Header -->
    <div class="border-b flex items-center justify-between px-6 py-4 shrink-0" style="background-color: var(--bg-card); border-color: var(--border-base)">
      <div class="flex items-center gap-4">
        <h1 class="text-xl font-semibold" style="color: var(--text-primary)">交流社区</h1>
        <span class="text-xs px-2.5 py-1 rounded-full font-medium" style="background-color: var(--bg-app); color: var(--text-muted)">{{ pagination.total }} 篇讨论</span>
      </div>
    ...
    <!-- Filter Bar -->
    <div class="px-6 py-3 border-b flex items-center gap-3 flex-wrap shrink-0" style="background-color: var(--bg-card); border-color: var(--border-base)">
      <!-- Sort Options -->
      ...
      <div class="max-w-4xl mx-auto space-y-3">
        ...
        <div v-if="filteredDiscussions.length > 0" class="space-y-3">
          <div v-for="d in filteredDiscussions" :key="d.id" 
            ...
            <!-- Pinned Badge -->
            <div v-if="d.isPinned" class="absolute top-3 right-3 flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold" style="background-color: var(--accent); color: white">
            ...
            <div class="flex gap-4">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1.5">
                  ...
                  <span class="text-[11px]" style="color: var(--text-muted)">{{ formatTime(d.createdAt) }}</span>
                </div>
                
                <h3 class="text-base font-bold mb-1.5 group-hover:text-accent transition-colors pr-16">{{ d.title }}</h3>
                <p class="text-sm line-clamp-2 mb-3" style="color: var(--text-secondary)">{{ d.content }}</p>

                <!-- Tags -->
                <div v-if="parseTags(d.tags).length > 0" class="flex flex-wrap gap-1.5 mb-3">
                  <span v-for="tag in parseTags(d.tags)" :key="tag" class="px-2 py-0.5 rounded-full text-[10px] font-medium" style="background-color: var(--bg-app); color: var(--accent)">#{{ tag }}</span>
                </div>
                
                <!-- Image Preview in List -->
                <div v-if="parseImages(d.images).length > 0" class="flex gap-2 mb-3 overflow-hidden h-20">
                  <div v-for="(img, idx) in parseImages(d.images).slice(0, 3)" :key="idx" class="w-20 h-20 rounded-lg overflow-hidden shrink-0 border border-slate-100 dark:border-white/5">
                    <img :src="img" class="w-full h-full object-cover" />
                  </div>
                  <div v-if="parseImages(d.images).length > 3" class="w-20 h-20 rounded-lg bg-slate-100 dark:bg-white/5 flex items-center justify-center text-xs font-bold text-slate-400">
                    +{{ parseImages(d.images).length - 3 }}
                  </div>
                </div>

                <div class="flex items-center gap-5 text-[11px] font-bold" style="color: var(--text-muted)">
                  <button 
                  ...
                  <!-- Admin/Owner Actions -->
                  <div v-if="currentUserId === d.user?.id || isAdmin" class="flex items-center gap-2 ml-auto">
                    <button v-if="isAdmin" @click.stop="togglePinDiscussion(d)" class="hover:text-accent transition-colors flex items-center gap-1">
                      <Pin class="w-3.5 h-3.5" />
                    </button>
                    <button @click.stop="deleteDiscussion(d)" class="hover:text-red-500 transition-colors flex items-center gap-1">
    ...
    <Transition name="fade">
      <div v-if="isDetailOpen && selectedDiscussion" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-3xl max-h-[90vh] flex flex-col rounded-3xl shadow-2xl overflow-hidden" style="background-color: var(--bg-card)">
          <!-- Modal Header -->
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="flex items-center gap-2">
                  ...
                  <span v-if="selectedDiscussion.isPinned" class="px-1.5 py-0.5 rounded text-[9px] font-bold" style="background-color: var(--accent); color: white">置顶</span>
                </div>
                <p class="text-[10px]" style="color: var(--text-muted)">{{ formatTime(selectedDiscussion.createdAt) }}</p>
              </div>
            </div>
            <div class="flex items-center gap-2">
              <button v-if="isAdmin" @click="togglePinDiscussion(selectedDiscussion)" class="p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-colors" style="color: var(--text-secondary)">
                <Pin class="w-4 h-4" :class="{ 'text-accent': selectedDiscussion.isPinned }" />
              </button>
              <button v-if="currentUserId === selectedDiscussion.user?.id || isAdmin" @click="deleteDiscussion(selectedDiscussion)" class="p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-colors" style="color: var(--text-secondary)">
                <Trash2 class="w-4 h-4" />
              </button>
              <button @click="isDetailOpen = false" class="p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-colors" style="color: var(--text-secondary)">
                <X class="w-5 h-5" />
              </button>
            </div>
          </div>
          ...
          <div class="flex-1 overflow-y-auto p-8 scrollbar-hide">
            <h2 class="text-2xl font-black mb-4" style="color: var(--text-primary)">{{ selectedDiscussion.title }}</h2>

            <!-- Tags -->
            <div v-if="parseTags(selectedDiscussion.tags).length > 0" class="flex flex-wrap gap-1.5 mb-4">
              <span v-for="tag in parseTags(selectedDiscussion.tags)" :key="tag" class="px-2.5 py-0.5 rounded-full text-[11px] font-medium" style="background-color: var(--bg-app); color: var(--accent)">#{{ tag }}</span>
            </div>

            <!-- Stats Bar -->
            <div class="flex items-center gap-5 mb-6 pb-4 border-b" style="border-color: var(--border-base)">
              <button 
              ...
              <div class="flex items-center gap-1.5 text-xs font-bold" style="color: var(--text-muted)">
            ...
            <!-- Post Images -->
            <div v-if="parseImages(selectedDiscussion.images).length > 0" class="grid grid-cols-1 gap-4 mb-10">
              <img v-for="(img, idx) in parseImages(selectedDiscussion.images)" :key="idx" :src="img" class="w-full rounded-2xl shadow-sm border border-slate-100 dark:border-white/5" />
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="flex gap-3">
                  ...
                  <div class="flex-1">
                    ...
                    <div class="flex items-center gap-4 mt-1.5 ml-2">
                      ...
                      <button 
                        @click="replyingTo = replyingTo?.id === comment.id ? null : comment; replyContent = ''" 
                    ...
                    <!-- Show Replies Toggle -->
                    <button 
                      v-if="comment._count?.replies > 0 && (!comment.replies || comment.replies.length === 0 || !expandedReplies.has(comment.id))"
                      ...
                      <div v-for="reply in comment.replies" :key="reply.id" class="flex gap-2.5">
                        ...
                        <div class="flex-1">
                          ...
                          <div class="flex items-center gap-3 mt-1 ml-1">
                            ...
                            >
                              <Heart class="w-2.5 h-2.5" :class="{ 'fill-red-500': reply.isLiked }" />
              ...
              <div v-if="!selectedDiscussion.comments?.length" class="py-8 text-center text-sm italic" style="color: var(--text-muted)">
                暂无回复，快来抢沙发吧！
              </div>
            </div>
          </div>
      ...
      <div v-if="showCreateModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-xl p-8 rounded-3xl shadow-2xl space-y-5" style="background-color: var(--bg-card)">
          ...
          <div class="space-y-4">
            ...
            <!-- Tags Input -->
            <div>
              <label class="block text-xs font-bold uppercase mb-2 ml-1" style="color: var(--text-secondary)">标签 (用逗号分隔)</label>
              <input v-model="postForm.tags" type="text" class="w-full px-4 py-3 border rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-accent/20 transition-all" style="background-color: var(--bg-app); border-color: var(--border-base); color: var(--text-primary)" placeholder="例如: 技术, 3D建模, Blender" />
              <div v-if="availableTags.length > 0" class="flex flex-wrap gap-1.5 mt-2">
                <span class="text-[10px] font-medium" style="color: var(--text-muted)">热门标签:</span>
                <button 
            ...
            <div>
              <label class="block text-xs font-bold uppercase mb-3 ml-1" style="color: var(--text-secondary)">图片 (最多 5 张)</label>
              <div class="flex flex-wrap gap-3">
                <div v-for="(img, idx) in imagePreviews" :key="idx" class="relative w-20 h-20 rounded-xl overflow-hidden group">
                  <img :src="img" class="w-full h-full object-cover" />
                  <button @click="removeImage(idx)" class="absolute top-1 right-1 p-1 bg-black/50 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                    <X class="w-3 h-3" />
                  </button>
                </div>
                <label v-if="imagePreviews.length < 5" class="w-20 h-20 rounded-xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5 transition-all" style="border-color: var(--border-base)">
                  <Plus class="w-5 h-5" style="color: var(--text-muted)" />
                  <span class="text-[10px] mt-1" style="color: var(--text-muted)">上传图片</span>
                  <input type="file" class="hidden" accept="image/*" multiple @change="handleImageSelect" />
                </label>
              </div>
            </div>
          </div>
