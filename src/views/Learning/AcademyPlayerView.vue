...
<template>
  <div class="flex h-full overflow-hidden bg-slate-950 text-slate-200">
    ...
    <div class="flex-1 flex flex-col relative overflow-hidden">
      <!-- Player Top Nav -->
      <div
        class="h-14 flex items-center justify-between px-6 bg-slate-900/50 backdrop-blur-md border-b border-white/5 z-20"
      >
      ...
      <div class="flex-1 relative bg-black flex items-center justify-center">
        ...
        <template v-else-if="currentLesson">
          ...
          <div v-if="is3DModel(currentLesson.videoUrl)" class="w-full h-full relative">
            ...
            />
            <div
              class="absolute bottom-6 right-6 bg-slate-900/80 backdrop-blur px-4 py-2 rounded-xl border border-white/10 pointer-events-none"
            >
          ...
          <div v-else class="w-full h-full p-8 md:p-16 overflow-y-auto bg-slate-900 scrollbar-hide">
            <div class="max-w-3xl mx-auto space-y-8">
              <div class="space-y-4">
                <span class="px-3 py-1 bg-accent/20 text-accent text-[10px] font-bold rounded-full"
                  >文字教程</span
                >
              ...
              <div class="pt-12 flex justify-center">
                <button
                  class="px-8 py-3 bg-accent text-white font-bold rounded-2xl shadow-xl shadow-accent/20 flex items-center gap-2"
      ...
      <div class="px-8 py-4 flex items-center justify-between bg-slate-900 border-t border-white/5">
        ...
        <div class="hidden md:flex items-center gap-6">
          <button
            v-for="tab in [
              { id: 'content' as const, icon: FileText, label: '课程内容' },
              { id: 'notes' as const, icon: StickyNote, label: '学习笔记' },
              { id: 'discussion' as const, icon: MessageSquare, label: '参与讨论' },
            ]"
            :key="tab.id"
          ...
          >
            ...
            <span
              ...
              class="px-1.5 py-0.5 rounded text-[9px] bg-accent/20 text-accent font-bold"
              >{{ notes.length }}</span
            >
          </button>
        </div>
        ...
        <div class="h-14 flex items-center justify-between px-6 border-b border-white/5 shrink-0">
          <div class="flex items-center gap-3">
            ...
            <span class="text-[10px] text-slate-500 font-bold"
              >{{ completedCount }}/{{ lessons.length }}</span
            >
          </div>
          <button class="lg:hidden p-2 hover:bg-white/10 rounded-lg" @click="isSidebarOpen = false">
            <X class="w-5 h-5" />
          </button>
        </div>
            ...
            <div class="p-4 space-y-2">
              ...
              >
                <div class="flex gap-4">
                  <div
                    class="shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black transition-colors"
                    :class="
                  ...
                  <div class="flex-1 min-w-0">
                    <h3
                      class="text-xs font-bold truncate mb-1"
                      :class="
                        currentLessonIndex === index
                          ? 'text-accent'
                          : isLessonCompleted(lesson.id)
            ...
            <div class="p-4 space-y-4">
              ...
              <div class="space-y-3">
                ...
                <textarea
                  ...
                  class="w-full px-4 py-3 rounded-xl bg-slate-800 border border-white/5 text-sm text-slate-200 placeholder-slate-600 outline-none resize-none focus:border-accent/30 transition-colors"
                ></textarea>
              ...
              <div class="space-y-2">
                ...
                >
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    <button
                      class="opacity-0 group-hover:opacity-100 p-1 rounded text-slate-500 hover:text-rose-400 transition-all"
