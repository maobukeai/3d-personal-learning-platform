...
export const encryptSecret = (value: string | null | undefined): string | null => {
  ...
  const activeSecret = secret || legacySecret;

  if (!activeSecret) {
    throw new Error(
      'Missing encryption key configuration. Please set DATABASE_ENCRYPTION_KEY in your .env file.',
    );
  }
  ...
  const key = getSecretKey(activeSecret);
  if (!key) return value;
