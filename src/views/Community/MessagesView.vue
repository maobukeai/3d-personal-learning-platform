...
const contextMenu = ref<{ visible: boolean; x: number; y: number; messageId: string; message: any }>({
  visible: false, x: 0, y: 0, messageId: '', message: null
})

const conversationContextMenu = ref<{ visible: boolean; x: number; y: number; conversation: any }>({
  visible: false, x: 0, y: 0, conversation: null
})
...
const filteredGroupUsers = computed(() => {
  ...
  return publicUsers.value.filter(u => {
    ...
    return !isAlreadyAdded && ((u.name || '').toLowerCase().includes(query) || u.email.toLowerCase().includes(query))
  })
})
...
const getConversationAvatar = (conv: any) => {
  if (!conv) return ''
  if (conv.isGroup && conv.avatarUrl) return conv.avatarUrl
  if (conv.isGroup) return ''
  const other = getOtherParticipant(conv)
  return other?.avatarUrl || `https://ui-avatars.com/api/?name=${other?.name || other?.email || 'U'}`
}
...
const getLastMessagePreview = (conv: any) => {
  ...
  switch (lastMsg.type) {
    ...
    case 'FILE': return `${prefix}[文件] ${lastMsg.content.split('/').pop()}`
...
const toggleReaction = async (messageId: string, emoji: string) => {
  ...
  try {
    ...
    if (!msg) return
    const existing = msg.reactions?.find((r: any) => r.emoji === emoji && r.userId === authStore.user?.id)
    if (existing) {
...
const getGroupedReactions = (reactions: any[]) => {
  ...
  const groups: Record<string, { emoji: string; count: number; hasMine: boolean; users: string[] }> = {}
  reactions.forEach(r => {
    if (!groups[r.emoji]) {
...
watch(() => isGroupChatDialogOpen.value, (val) => {
  if (val && publicUsers.value.length === 0) fetchPublicUsers()
})
</script>

<template>
  <div class="flex-1 flex h-full overflow-hidden" style="background-color: var(--bg-app)" @click="closeContextMenu(); closeConversationContextMenu()">
    <!-- Contacts Sidebar -->
      ...
      <div class="p-6 border-b" style="border-color: var(--border-base)">
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="flex gap-1">
            ...
            <button @click="isNewChatDialogOpen = true" class="p-2 bg-accent/10 text-accent rounded-xl hover:bg-accent hover:text-white transition-all" title="发起新聊天">
              <Plus class="w-4 h-4" />
            </button>
          </div>
        </div>
    ...
    <div class="flex-1 flex flex-col relative" style="background-color: var(--bg-app)">
      ...
      <template v-if="activeConversation">
        ...
        <div class="h-16 border-b px-6 flex items-center justify-between shrink-0" style="background-color: var(--bg-card); border-color: var(--border-base)">
          <div class="flex items-center gap-3">
            <template v-if="activeConversation.isGroup">
              <div class="w-10 h-10 rounded-full flex items-center justify-center" :class="activeConversation.avatarUrl ? '' : 'bg-indigo-500/10'">
                <img v-if="activeConversation.avatarUrl" :src="activeConversation.avatarUrl" class="w-10 h-10 rounded-full object-cover" />
                <Users v-else class="w-5 h-5 text-indigo-500" />
              </div>
            </template>
            <template v-else>
        ...
        <div ref="messagesContainer" 
          ...
          <template v-for="(msg, index) in filteredMessages" :key="msg.id">
            <!-- Date Separator -->
            <div v-if="shouldShowDateSeparator(msg, filteredMessages[index - 1])" class="flex items-center justify-center my-6">
              <div class="px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider" style="background-color: var(--bg-card); color: var(--text-muted); border: 1px solid var(--border-base)">
            ...
            <div v-if="msg.type === 'SYSTEM'" class="flex items-center justify-center my-3">
              <div class="px-4 py-1.5 rounded-full text-[10px] font-medium" style="background-color: var(--bg-card); color: var(--text-muted)">
                {{ msg.content }}
              </div>
            </div>
            ...
            <div v-else class="flex group" :class="msg.senderId === authStore.user?.id ? 'justify-end' : 'justify-start'">
              <div class="flex gap-3 max-w-[70%]" :class="msg.senderId === authStore.user?.id ? 'flex-row-reverse' : ''">
                ...
                <div class="flex flex-col relative" :class="msg.senderId === authStore.user?.id ? 'items-end' : 'items-start'">
                  ...
                  <p v-if="activeConversation?.isGroup && msg.senderId !== authStore.user?.id && shouldShowSenderAvatar(msg, filteredMessages[index - 1])" 
                     class="text-[10px] font-bold mb-1 px-1" style="color: var(--text-muted)">
                    {{ msg.sender?.name || '未知' }}
                  </p>

                  <!-- Reply Preview -->
                  <div v-if="msg.replyTo" class="mb-1 px-3 py-1.5 rounded-xl text-[10px] max-w-full border-l-2 border-accent" style="background-color: var(--bg-card); color: var(--text-secondary)">
                    <span class="font-bold text-accent">{{ msg.replyTo.sender?.name || '未知' }}</span>
                    <span class="ml-1 truncate inline-block max-w-[200px] align-bottom">{{ msg.replyTo.type === 'IMAGE' ? '[图片]' : msg.replyTo.type === 'FILE' ? '[文件]' : msg.replyTo.content }}</span>
                  </div>
                  ...
                  <!-- Reactions Display -->
                  <div v-if="msg.reactions && getGroupedReactions(msg.reactions).length > 0" class="flex flex-wrap gap-1 mt-1">
                    <button 
                      ...
                      :style="!group.hasMine ? 'background-color: var(--bg-card); color: var(--text-secondary); border: 1px solid var(--border-base)' : ''"
                  ...
                  <!-- Timestamp -->
                  <div v-if="shouldShowTimestamp(msg, filteredMessages[index + 1])" class="flex items-center gap-1 mt-1 px-1">
        ...
        <!-- Reply Bar -->
        <div v-if="replyToMessage" class="px-6 py-2 border-t flex items-center gap-3" style="background-color: var(--bg-card); border-color: var(--border-base)">
          <Reply class="w-4 h-4 text-accent shrink-0" />
          <div class="flex-1 min-w-0">
            <p class="text-[10px] font-bold text-accent">{{ replyToMessage.sender?.name || '未知' }}</p>
            <p class="text-xs truncate" style="color: var(--text-secondary)">{{ replyToMessage.type === 'IMAGE' ? '[图片]' : replyToMessage.type === 'FILE' ? '[文件]' : replyToMessage.content }}</p>
          </div>
          <button @click="cancelReply" class="p-1 hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg transition-all">
      ...
      <div v-else class="flex-1 flex flex-col items-center justify-center p-12 text-center" style="background-color: var(--bg-app)">
        ...
        <div class="flex gap-3">
          ...
          <button @click="isGroupChatDialogOpen = true" class="px-8 py-3 bg-indigo-500 text-white rounded-2xl font-bold shadow-lg shadow-indigo-500/20 hover:scale-105 transition-all">
            创建群聊
          </button>
        </div>
      </div>
    ...
    <div v-if="contextMenu.visible" 
      ...
      <div class="px-3 py-2 flex gap-1">
        <button v-for="emoji in reactionEmojis.slice(0, 6)" :key="emoji" 
                @click="toggleReaction(contextMenu.messageId, emoji); closeContextMenu()"
                class="w-8 h-8 flex items-center justify-center text-base hover:scale-125 hover:bg-accent/10 rounded-lg transition-all">
          {{ emoji }}
        </button>
      </div>
    ...
    <div v-if="conversationContextMenu.visible" 
         class="fixed z-[100] py-2 rounded-2xl shadow-2xl border min-w-[160px]"
    ...
    >
      <div class="space-y-5">
        <div>
          <label class="text-xs font-bold uppercase tracking-wider mb-2 block" style="color: var(--text-muted)">群聊名称</label>
          <input 
        ...
        <div v-if="groupChatParticipants.length > 0">
          <label class="text-xs font-bold uppercase tracking-wider mb-2 block" style="color: var(--text-muted)">已选择 ({{ groupChatParticipants.length }})</label>
        ...
        <div>
          <label class="text-xs font-bold uppercase tracking-wider mb-2 block" style="color: var(--text-muted)">添加成员</label>
          ...
          <div class="max-h-60 overflow-y-auto scrollbar-hide space-y-2">
            ...
            >
              ...
              <div class="w-8 h-8 rounded-full bg-indigo-500/5 flex items-center justify-center text-indigo-500 group-hover:bg-indigo-500 group-hover:text-white transition-all">
                <Plus class="w-4 h-4" />
              </div>
            </div>
          </div>
