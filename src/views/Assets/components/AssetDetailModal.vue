...
const handleDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      label(
        `确定要删除资源「${asset.value.title}」吗？此操作不可恢复。`,
        `Are you sure you want to delete asset "${asset.value.title}"? This action cannot be undone.`,
      ),
...
const {
  ...
  canDeleteComment,
  resetComments,
} = useResourceComments('assets', () => asset.value?.id || props.assetId);
...
const handleDownload = async (isPackage = false) => {
  ...
  if (!canDownload.value) {
    ElMessage.warning(
      label(
        '该资源为会员专享，请先升级为会员后再下载。',
        'This resource is VIP-only. Please upgrade to download.',
      ),
    );
    return;
  }
  ...
  const totalSizeOverrideBytes =
    (isPackage ? asset.value.packageSize || 0 : asset.value.size || 0) * 1024 * 1024;
  ...
  await runDownload({
    ...
    onError: (err) => {
      const msg =
        ...
        label('下载失败，您可能需要会员权限', 'Download failed, you may need VIP permission');
      ElMessage.error(msg);
    },
  });
};
  ...
  <component
    ...
    v-bind="
      inline
        ...
        : { show: show, size: 'xxl', padding: 'md', glassCard: true }
    "
    @close="emit('close')"
  >
    ...
    >
      ...
      <span class="text-xs font-semibold tracking-wider uppercase">{{
        label('正在载入三维档案...', 'Loading 3D asset...')
      }}</span>
    </div>
    ...
    <div v-else-if="asset" class="grid grid-cols-1 xl:grid-cols-12 gap-6">
      ...
      >
          ...
          <div class="flex flex-col gap-2.5">
            ...
            >
              ...
              <FolderOpen
                ...
                class="h-4.5 w-4.5 animate-bounce-slow text-white"
              />
              <span>
                {{ label('下载源文件包 (.zip)', 'Download Source (.zip)') }}
                <span v-if="!canDownload" class="text-[10px] text-amber-300 font-bold ml-1"
                  >(VIP)</span
                >
              </span>
            </Button>
        ...
        <div
          ...
          class="border border-white/10 rounded-xl overflow-hidden bg-white/[0.01] dark:bg-white/[0.02] text-left my-2 shrink-0"
        >
          <div
        ...
        <div class="grid grid-cols-2 gap-2 text-xs">
          <!-- File size -->
          <div
            class="p-3 bg-white/[0.01] border border-white/5 rounded-xl flex flex-col gap-1 text-left"
          >
