...
const taskForm = ref({ title: '', description: '', assigneeId: '', dueDate: '', participantIds: [] as string[] })
const isEditTaskDialogOpen = ref(false)
const editingTask = ref<any>(null)
const editTaskForm = ref({ title: '', description: '', assigneeId: '', dueDate: '', status: '', participantIds: [] as string[] })
...
const handleReaction = async (discussionId: string, emoji: string) => {
  try {
    ...
    if (msg) {
      const existingIdx = msg.reactions?.findIndex((r: any) => r.userId === authStore.user?.id && r.emoji === emoji)
      if (existingIdx >= 0) {
        msg.reactions.splice(existingIdx, 1)
      } else {
        if (!msg.reactions) msg.reactions = []
        msg.reactions.push({ emoji, userId: authStore.user?.id, user: { id: authStore.user?.id, name: authStore.user?.name, avatarUrl: authStore.user?.avatarUrl } })
      }
    }
  } catch {
    // silently fail
...
const handleCreateTask = async () => {
  ...
  try {
    ...
    taskForm.value = { title: '', description: '', assigneeId: '', dueDate: '', participantIds: [] }
    fetchProject()
...
const handleUpdateTask = async () => {
  ...
  try {
    const response = await api.put(`/api/projects/tasks/${editingTask.value.id}`, editTaskForm.value)
...
const updateTaskStatus = async (task: any, newStatus: string) => {
  ...
  try {
    const response = await api.put(`/api/projects/tasks/${task.id}`, { ...task, status: newStatus })
    if (response.data._projectProgress !== undefined) {
      project.value.progress = response.data._projectProgress
    }
  } catch (error) {
...
const tasksByStatus = computed(() => {
  ...
  if (taskDateFilter.value !== 'all') {
    ...
    if (taskDateFilter.value === 'overdue') {
      filtered = filtered.filter((t: any) => t.dueDate && new Date(t.dueDate) < now && t.status !== 'DONE')
    } else if (taskDateFilter.value === 'today') {
      const endOfDay = new Date(now)
      endOfDay.setHours(23, 59, 59, 999)
      filtered = filtered.filter((t: any) => t.dueDate && new Date(t.dueDate) >= now && new Date(t.dueDate) <= endOfDay)
    } else if (taskDateFilter.value === 'week') {
      const endOfWeek = new Date(now)
      endOfWeek.setDate(now.getDate() + 7)
      filtered = filtered.filter((t: any) => t.dueDate && new Date(t.dueDate) >= now && new Date(t.dueDate) <= endOfWeek)
    }
  }
...
<template>
  <div class="flex-1 flex overflow-hidden bg-slate-50 dark:bg-slate-950 font-sans relative">
    ...
    <!-- Global Loading -->
    <div v-if="isLoading" class="absolute inset-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm flex items-center justify-center">
    ...
    <template v-else-if="project">
      ...
      <aside class="w-96 bg-white dark:bg-slate-900 border-r flex flex-col shrink-0 transition-colors z-10 shadow-[5px_0_30px_rgba(0,0,0,0.02)]" style="border-color: var(--border-base)">
        ...
        <div class="p-8 pb-4">
          ...
          <div class="flex items-start gap-4 mb-4">
            <div class="w-14 h-14 rounded-[1rem] flex items-center justify-center text-white text-2xl font-black shadow-lg shrink-0" :class="project.color">
          ...
          <div class="flex flex-wrap gap-2 mb-8">
            <span v-for="tag in getTagsList(project.tags)" :key="tag" class="text-xs font-bold px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-lg text-slate-500">
              #{{ tag }}
            </span>
          </div>

          <!-- Progress -->
          <div class="space-y-3 mb-8 p-5 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border" style="border-color: var(--border-base)">
      ...
      <main class="flex-1 flex flex-col min-w-0 bg-transparent">
        ...
        <!-- Top Navigation -->
        <header class="h-20 px-10 flex items-center justify-between border-b bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl shrink-0" style="border-color: var(--border-base)">
        ...
        <div v-if="activeTab === 'tasks'" class="px-10 py-3 border-b flex items-center gap-6 overflow-x-auto scrollbar-hide shrink-0" style="border-color: var(--border-base)">
          <div class="flex items-center gap-2">
        ...
        <div class="flex-1 overflow-hidden relative">
          ...
          <div v-show="activeTab === 'tasks'" class="absolute inset-0 p-8 overflow-x-auto">
            <div class="flex gap-8 h-full pb-4 items-start min-w-max">
              ...
              <div v-for="col in [
                ...
                :class="[col.color, dragOverColumn === col.id ? 'bg-slate-200/50 dark:bg-slate-700/50 scale-[1.02]' : '']"
              ...
              >
                ...
                <div class="px-6 py-5 flex items-center justify-between shrink-0">
                  <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-sm font-black text-slate-700 dark:text-slate-200">{{ col.label }}</h3>
                  </div>
                  <span class="px-2 py-0.5 rounded-md bg-white dark:bg-slate-800 text-xs font-bold text-slate-500 shadow-sm">{{ (tasksByStatus as Record<string, any[]>)[col.id].length }}</span>
                </div>
                ...
                <div class="flex-1 overflow-y-auto px-4 pb-4 space-y-4 scrollbar-hide">
                  <div v-for="task in (tasksByStatus as Record<string, any[]>)[col.id]" :key="task.id" 
                    ...
                    <p class="text-xs text-slate-500 mb-5 line-clamp-2 leading-relaxed">{{ task.description || '无详细描述' }}</p>
                  ...
                  <div v-if="(tasksByStatus as Record<string, any[]>)[col.id].length === 0" class="h-32 rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-600 flex items-center justify-center text-slate-400 text-xs font-bold opacity-50">
                    拖拽任务至此
                  </div>
                </div>
              </div>
          ...
          <div v-show="activeTab === 'discussions'" class="absolute inset-0 flex flex-col bg-white dark:bg-slate-950">
            <template v-if="isMember">
              <div ref="chatScroll" class="flex-1 overflow-y-auto p-10 space-y-8 scrollbar-hide">
                <div v-for="msg in project.discussions" :key="msg.id" 
                  ...
                  <div :class="msg.userId === authStore.user?.id ? 'items-end' : ''" class="flex flex-col">
                    ...
                    <div v-if="parseImages(msg.images).length > 0" class="mt-2 flex flex-wrap gap-2 max-w-xl">
                      <img v-for="(img, idx) in parseImages(msg.images)" :key="idx" :src="img" class="max-w-[200px] max-h-[200px] rounded-2xl object-cover cursor-pointer hover:opacity-80 transition-opacity" @click="openImage(img)" />
                    </div>

                    <div v-if="msg.fileUrl" class="mt-2 max-w-xl">
                      <a :href="msg.fileUrl" target="_blank" class="flex items-center gap-3 px-4 py-3 rounded-2xl bg-white dark:bg-slate-800 border shadow-sm hover:shadow-md transition-all" style="border-color: var(--border-base)">
                        <Paperclip class="w-5 h-5 text-accent shrink-0" />
                        <div class="min-w-0 flex-1">
                          <p class="text-sm font-bold truncate" style="color: var(--text-primary)">{{ msg.fileName || '文件' }}</p>
                          <p v-if="msg.fileSize" class="text-[10px] text-slate-400">{{ formatFileSize(msg.fileSize) }}</p>
                        </div>
                      </a>
                    </div>
                ...
                <div v-if="project.discussions.length === 0" class="h-full flex flex-col items-center justify-center text-slate-400 opacity-50">
              ...
              <!-- Input Area -->
              <div class="p-6 bg-white dark:bg-slate-950 border-t" style="border-color: var(--border-base)">
                <!-- Image Preview -->
                ...
                <!-- File Preview -->
                <div v-if="selectedFile" class="flex items-center gap-3 mb-3 px-4 py-2 bg-slate-50 dark:bg-slate-900 rounded-xl">
                ...
                <div class="max-w-4xl mx-auto relative">
                  ...
                  <div class="flex items-center gap-1 mb-2 pl-2">
                    <label class="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer transition-all text-slate-400 hover:text-accent">
            ...
            <div v-else class="h-full flex flex-col items-center justify-center text-center p-12 bg-slate-50 dark:bg-slate-900/50">
              <div class="w-24 h-24 rounded-full bg-slate-200 dark:bg-slate-800 flex items-center justify-center mb-8">
    ...
    >
      ...
      <div class="space-y-6 p-2">
        <div>
          <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 ml-1">任务名称</label>
          <input v-model="taskForm.title" type="text" class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border rounded-2xl text-sm focus:outline-none focus:ring-4 focus:ring-accent/10 transition-all font-bold" style="border-color: var(--border-base); color: var(--text-primary)" placeholder="简明扼要..." />
        </div>
        
        <div>
          <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 ml-1">执行标准 (描述)</label>
          <textarea v-model="taskForm.description" rows="4" class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border rounded-2xl text-sm focus:outline-none focus:ring-4 focus:ring-accent/10 transition-all resize-none" style="border-color: var(--border-base); color: var(--text-primary)" placeholder="交付物包含哪些内容？"></textarea>
        </div>

        <div>
          <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 ml-1">委派给</label>
        ...
        <div>
          <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 ml-1">参与人员</label>
        ...
        <div>
          <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 ml-1">Deadline</label>
          <el-date-picker
    ...
    >
      ...
      <div class="space-y-6 p-2">
        <div>
          <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 ml-1">任务名称</label>
          <input v-model="editTaskForm.title" type="text" class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border rounded-2xl text-sm focus:outline-none focus:ring-4 focus:ring-accent/10 transition-all font-bold" style="border-color: var(--border-base); color: var(--text-primary)" />
        </div>
        
        <div>
          <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 ml-1">任务描述</label>
          <textarea v-model="editTaskForm.description" rows="4" class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border rounded-2xl text-sm focus:outline-none focus:ring-4 focus:ring-accent/10 transition-all resize-none" style="border-color: var(--border-base); color: var(--text-primary)"></textarea>
        </div>
    ...
    <!-- Invite Dialog -->
    <el-dialog v-model="showInviteDialog" title="邀请成员" width="440px" class="custom-dialog" :show-close="false">
      <template #header="{ close }">
        <div class="flex items-center justify-between">
      ...
      <div class="space-y-4">
        ...
        <div v-else>
          <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 ml-1">选择要邀请的成员</label>
