...
<template>
  ...
  <div class="mirror-resource-detail h-full overflow-y-auto p-4 md:p-6 w-full max-w-[1400px] mx-auto scrollbar-hide">
    ...
    <div class="flex items-center gap-3 mb-6">
      ...
      <button type="button" class="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors" @click="goBack">
    ...
    <div v-else-if="error" class="text-center py-20">
      ...
      <button type="button" class="mt-4 px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-sm text-slate-600 hover:bg-slate-200 transition-colors" @click="goBack">
    ...
    <template v-else-if="resource">
      ...
      <div v-if="!authStore.isAuthenticated" class="mb-6 p-4 rounded-xl bg-blue-50/50 dark:bg-blue-500/5 border border-blue-200/50 dark:border-blue-500/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button type="button" class="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold shrink-0 shadow-sm transition-colors cursor-pointer" @click="router.push(`/login?redirect=${route.fullPath}`)">
      ...
      <div v-else-if="resource.hasAccess === false" class="mb-6 p-4 rounded-xl bg-amber-50/50 dark:bg-amber-500/5 border border-amber-200/50 dark:border-amber-500/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button type="button" class="px-4 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded-lg text-xs font-semibold shrink-0 shadow-sm transition-colors cursor-pointer" @click="router.push('/billing')">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700/50 overflow-hidden">
            ...
            <div v-if="resource.thumbnailUrl" class="w-full aspect-[16/9] max-h-[500px] bg-slate-100 dark:bg-slate-700 overflow-hidden">
              ...
              <img :src="getAssetUrl(resource.thumbnailUrl)" :alt="resource.title" class="w-full h-full object-cover" @error="($event.target as HTMLImageElement).style.display = 'none'" />
...
type="button" :class="[
                          ...
                          ]" :disabled="isExtracting || (authStore.isAuthenticated && resource.hasAccess === undefined)" @click="handleExtract(link)">
                ...
                <div class="mt-4 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 rounded-xl p-4 flex items-center justify-between">
                  ...
                  <button type="button" :disabled="isTogglingLike" class="flex items-center gap-1.5 px-4 py-2 rounded-lg transition-all duration-300 border" :class="likeStatus.liked ? 'bg-red-50 dark:bg-red-500/10 text-red-500 border-red-500/20' : 'bg-white dark:bg-slate-900 text-slate-500 border-slate-200 dark:border-slate-800'" @click="toggleLike">
              ...
              <SafeHtml v-if="resource.contentHtml" class="mirror-content prose prose-sm dark:prose-invert max-w-none" :html="stripManualDownloadLink(resource.contentHtml)" />
            ...
            <div class="flex gap-3">
              ...
              <div class="flex-1 space-y-3">
                ...
                <div class="flex justify-end">
                  ...
                  <button type="button" :disabled="isSubmittingComment || !newCommentText.trim()" class="px-4 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold text-xs transition-colors flex items-center gap-1.5" @click="submitComment">
            ...
            <div v-else class="space-y-4 divide-y divide-slate-100 dark:divide-slate-800">
              ...
              >
                ...
                <div class="w-8 h-8 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 shrink-0">
                  ...
                  <img v-if="comment.user?.avatarUrl" alt="" :src="getAssetUrl(comment.user.avatarUrl)" class="w-full h-full object-cover" />
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div v-if="comment.userId === authStore.user?.id || authStore.user?.role === 'ADMIN'" class="flex justify-end mt-1.5">
                    ...
                    <button type="button" class="text-[10px] text-rose-500 hover:text-rose-600 transition-colors flex items-center gap-1" @click="deleteComment(comment.id)">
      ...
      >
        ...
        <div class="glass-dialog rounded-xl w-full max-w-md mx-4 shadow-2xl overflow-hidden">
          ...
          <div class="flex items-center justify-between p-5 border-b border-black/5 dark:border-white/5">
            ...
            <button type="button" class="p-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-black/5 dark:hover:bg-white/5 transition-colors cursor-pointer" @click="showLinkDialog = false">
          ...
          <div class="p-5 space-y-4">
            ...
            <div class="space-y-1.5">
              ...
              <div class="flex gap-2">
                ...
                <button type="button" class="px-3 py-2 text-xs font-semibold rounded-lg bg-black/5 hover:bg-black/10 dark:bg-white/5 dark:hover:bg-white/10 text-slate-700 dark:text-slate-200 transition-colors border border-black/10 dark:border-white/10 cursor-pointer" @click="copyToClipboard(activeLink.url); ElMessage.success('下载链接已复制到剪贴板！')">
            ...
            <div v-if="activeLink.code" class="space-y-1.5">
              ...
              <div class="flex gap-2">
                ...
                <button type="button" class="px-3 py-2 text-xs font-semibold rounded-lg bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 text-red-500 transition-colors border border-red-500/20 cursor-pointer" @click="copyToClipboard(activeLink.code)">
          ...
          <div class="flex gap-2 p-5 border-t border-black/5 dark:border-white/5 bg-black/[0.02] dark:bg-white/[0.02]">
            ...
            <button type="button" class="flex-1 py-2 rounded-lg border border-slate-200 dark:border-slate-600 text-xs text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 font-semibold transition-colors cursor-pointer" @click="showLinkDialog = false">
