...
const cachedRoadmaps = ref<any[]>([]);
const cachedProgress = ref<any[]>([]);
const cachedCourses = ref<any[]>([]);
...
const roadmaps = ref<Roadmap[]>(cachedRoadmaps.value);
const myProgress = ref<RoadmapProgress[]>(cachedProgress.value);
const allCourses = ref<Course[]>(cachedCourses.value);
...
// Initialize selectedRoadmap from cache if available
...
const fetchData = async () => {
  ...
  try {
    ...
    if (selectedRoadmap.value) {
      const match = fetchedRoadmaps.find((r: Roadmap) => r.id === selectedRoadmap.value?.id);
      if (match) {
