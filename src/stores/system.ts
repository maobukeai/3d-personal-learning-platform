    ...
    async fetchSettings() {
      try {
        ...
        this.settings = {
          ...
          MATERIAL_CATEGORIES: data.MATERIAL_CATEGORIES || ['全部材料', '金属', '木纹', '石材', '织物', '程序化', '玻璃', '其他'],
          PASSWORD_MIN_LENGTH: data.PASSWORD_MIN_LENGTH || '6',
          SESSION_TIMEOUT: data.SESSION_TIMEOUT || '7d',
          AUTO_APPROVE_MATERIALS: data.AUTO_APPROVE_MATERIALS === 'true',
          AUTO_APPROVE_SHOWCASES: data.AUTO_APPROVE_SHOWCASES === 'true',
          MAX_UPLOAD_SIZE_MB: data.MAX_UPLOAD_SIZE_MB || '100',
          ALLOWED_FILE_TYPES: data.ALLOWED_FILE_TYPES || ['.glb', '.gltf', '.fbx', '.obj', '.stl', '.zip'],
