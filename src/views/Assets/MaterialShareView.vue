...
const material = ref<any | null>(null);
...
const loadSharedMaterial = async () => {
  ...
  try {
    const res = await api.get(`/api/materials/share/${shareId}?t=${Date.now()}`);
    material.value = res.data.material;
    customText.value = res.data.customText;

    document.title = `${res.data.material.title} | ${systemStore.settings.PLATFORM_NAME || '3D 学习平台'}`;
  } catch (error: any) {
    if (error.response?.status === 410) {
      isExpired.value = true;
      errorMsg.value = '该分享链接已过期失效';
    } else {
      errorMsg.value = error.response?.data?.error || '无法加载分享的材质，链接可能不存在或已被取消';
    }
  ...
  >
    ...
    >
      <div class="flex items-center gap-2 cursor-pointer select-none" @click="goHome">
        <div
          ...
          :class="
            systemStore.settings.PLATFORM_LOGO_URL && !logoLoadFailed
              ? 'bg-transparent'
              : 'bg-gradient-to-tr from-indigo-500 to-cyan-500 shadow-sm'
          "
        >
          <img
        ...
        <span
          class="text-xs font-black bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-cyan-600 dark:from-indigo-400 dark:to-cyan-400"
        >
    ...
    <div
      v-if="customText"
      class="bg-gradient-to-r from-indigo-500/[0.04] to-cyan-500/[0.04] border-b border-indigo-500/10 px-6 py-2.5 text-center text-xs font-bold text-indigo-600 dark:text-indigo-400 flex items-center justify-center gap-2 shrink-0"
    >
    ...
    <!-- Main Content Grid -->
    <main
      class="flex-1 max-w-[1440px] w-full mx-auto px-2 sm:px-6 py-6 lg:py-10 flex flex-col lg:flex-row gap-6 relative z-10 items-stretch"
    >
      ...
      >
        ...
        <p class="text-xs text-[var(--text-secondary)] max-w-sm mb-6 leading-relaxed">
          {{ errorMsg || '该材质共享可能已到期，或已被作者停止公共访问。' }}
        </p>
      ...
      <!-- Main Layout columns when loaded -->
      <template v-else-if="material">
        <GlassCard padding="none" class="shadow-xl p-3 sm:p-6 w-full">
          <MaterialDetailPanel :material="material" :loading="loading" inline />
        </GlassCard>
      </template>
    </main>
  </div>
