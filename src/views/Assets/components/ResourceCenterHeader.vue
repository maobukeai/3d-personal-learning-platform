...
<template>
  <PageHeader
    ...
    :subtitle="label('统一查看资源、材质、插件和作品展示的运营状态。', 'Unify viewing of operational status for resources, materials, plug-ins and showcases.')"
    :icon="FileStack"
  >
    <template #center>
      <label class="search-box !min-h-0 !h-8 w-44 sm:w-64 md:w-80 shrink-0">
        <Search />
        <input v-model="searchQuery" type="text" :placeholder="label('搜索最近内容、作者或标签', 'Search recent content, author, or tags')" />
      </label>
    </template>
