...
<template>
  ...
  >
    <div v-if="props.group" class="relative">
      ...
      <div class="flex flex-col md:flex-row h-[600px] overflow-hidden rounded-[24px] bg-white dark:bg-slate-900 shadow-2xl">
        <!-- Left: Image & Quick Stats -->
        <div class="md:w-5/12 relative">
          <img :src="props.group.avatarUrl || props.group.image || `https://images.unsplash.com/photo-1614850523296-d8c1af93d400?w=500&q=80`" class="w-full h-full object-cover" :alt="props.group.name">
          ...
          <div class="absolute bottom-8 left-8 right-8 text-white">
            ...
            <div class="flex items-center gap-6 mt-6">
              <div class="flex flex-col">
                ...
                <span class="text-lg font-black mt-1">{{ props.group._count?.members || props.group.members || 1 }}</span>
              </div>
              <div class="flex flex-col">
          ...
          <div class="space-y-8">
            <section>
              ...
              <p class="text-slate-600 dark:text-slate-300 leading-relaxed font-medium">
                {{ props.group.description || '这是一个充满活力的学习社区，我们不仅分享最前沿的 3D 技术，更注重通过实战项目提升每位成员的创作能力。加入我们，与全球优秀的创作者共同进步。' }}
              </p>
            </section>

            <section>
              <h3 class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">核心价值</h3>
              <div class="grid grid-cols-1 gap-3">
                <div v-for="(val, i) in ['技术共享', '协同创作', '共同成长']" :key="i" class="flex items-start gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border-2 border-transparent hover:border-accent/10 transition-all group">
                  <div class="w-10 h-10 rounded-xl bg-accent/10 flex items-center justify-center shrink-0 group-hover:bg-accent transition-colors">
                  ...
                  <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">涵盖了从基础理论到高级商业案例的完整闭环流程。</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section>
              <h3 class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">活跃成员</h3>
              <div class="flex items-center gap-3">
                <div class="flex -space-x-3">
                  <template v-if="props.group.members?.length">
                    <UserAvatar v-for="member in props.group.members" :key="member.id" 
                      :user="member.user"
                      size="md"
                      class="ring-4 ring-white dark:ring-slate-900"
                    />
                  </template>
                  <div v-else class="w-10 h-10 rounded-full border-4 border-white dark:border-slate-900 bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
