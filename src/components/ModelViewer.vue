...
const envMaps: Record<string, string> = {
  'sunset': 'https://raw.githubusercontent.com/mrdoob/three.js/dev/examples/textures/equirectangular/venice_sunset_1k.hdr',
  'studio': 'https://raw.githubusercontent.com/mrdoob/three.js/dev/examples/textures/equirectangular/royal_esplanade_1k.hdr',
  'forest': 'https://raw.githubusercontent.com/mrdoob/three.js/dev/examples/textures/equirectangular/pedestrian_overpass_1k.hdr',
...
const flyTo = (position: { x: number, y: number, z: number }, target: { x: number, y: number, z: number }) => {
...
const calculateStats = (model: THREE.Object3D, animCount: number = 0) => {
  ...
  model.traverse((child) => {
    if (child instanceof THREE.Mesh) {
      ...
      fCount += child.geometry.index ? child.geometry.index.count / 3 : child.geometry.attributes.position.count / 3
    }
  })
  stats.value = {
    vertices: vCount,
...
watch(() => props.modelUrl, (newUrl) => { if (newUrl) loadModel(newUrl) })
watch(() => props.autoRotate, (val) => { if (controls) controls.autoRotate = !!val })
watch(() => props.sceneConfig, () => { updateSceneConfig() }, { deep: true })

defineExpose({ getCameraState, flyTo })
</script>
...
<template>
  <div ref="container" class="w-full h-full cursor-move relative overflow-hidden group">
    ...
    <template v-for="(h, i) in hotspotsWithScreenPos" :key="i">
      <div v-if="h.isVisible" class="absolute z-30 transition-transform duration-200 pointer-events-none" :style="{ transform: `translate(${h.screenX}px, ${h.screenY}px)` }">
        <div class="relative -translate-x-1/2 -translate-y-1/2">
          <button @click.stop="handleHotspotClick(i)" class="w-6 h-6 bg-accent border-2 border-white rounded-full shadow-lg flex items-center justify-center text-white hover:scale-110 transition-transform pointer-events-auto group/dot">
    ...
    <div class="absolute right-4 top-4 flex flex-col gap-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <button @click="toggleFullscreen" class="p-2 bg-slate-900/80 backdrop-blur-md border border-white/10 rounded-lg text-white hover:bg-accent transition-colors">
