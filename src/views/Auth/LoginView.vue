  ...
  <div class="min-h-screen flex font-sans overflow-hidden" style="background-color: var(--bg-card)">
    ...
    <div class="hidden lg:flex w-1/2 bg-slate-900 relative overflow-hidden">
      ...
      <div class="relative z-10 p-16 flex flex-col w-full h-full">
        ...
        <div class="flex-1 flex flex-col justify-center">
          <div class="max-w-md">
            <h2 class="text-4xl font-bold text-white mb-6 leading-tight">开启你的 <span class="text-accent">3D 创意</span> 学习之旅</h2>
      ...
      <div class="absolute right-[-10%] top-1/2 -translate-y-1/2 w-96 h-96 bg-gradient-to-br from-accent/20 to-indigo-500/20 rounded-full blur-3xl"></div>
    </div>
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-8 md:p-16">
      <div class="w-full max-w-md">
        ...
        <div class="mb-10">
          <h1 class="text-3xl font-bold mb-2" style="color: var(--text-primary)">{{ is2FARequired ? '两步验证' : '欢迎回来' }}</h1>
          <p style="color: var(--text-secondary)">{{ is2FARequired ? '请输入 Google Authenticator 中的 6 位验证码' : '请输入你的账号信息以登录平台' }}</p>
        </div>

        <div v-if="!is2FARequired" class="space-y-6">
          <!-- Social Logins -->
          <div class="grid grid-cols-2 gap-4">
            <button class="flex items-center justify-center gap-2 py-2.5 border rounded-xl text-sm font-bold transition-all"
          ...
          <div class="relative py-4 flex items-center">
            ...
            <span class="flex-shrink mx-4 text-xs font-bold uppercase tracking-widest" style="color: var(--text-muted)">或者使用邮箱</span>
            <div class="flex-grow border-t" style="border-color: var(--border-base)"></div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div>
              <div class="flex items-center justify-between mb-2 ml-1">
                <label class="block text-xs font-bold uppercase" style="color: var(--text-secondary)">密码</label>
                <RouterLink to="/forgot-password" class="text-xs font-bold text-accent hover:text-accent transition-colors">忘记密码？</RouterLink>
              </div>
              <div class="relative">
          ...
          <p class="text-center text-sm" style="color: var(--text-secondary)">
            还没有账号？ 
            <RouterLink to="/register" class="font-bold text-accent hover:text-accent transition-colors">立即免费注册</RouterLink>
          </p>
        </div>
        ...
        <div v-else class="space-y-6">
          ...
          <div>
            <label class="block text-xs font-bold uppercase mb-2 ml-1 text-center" style="color: var(--text-secondary)">动态验证码</label>
            <input 
