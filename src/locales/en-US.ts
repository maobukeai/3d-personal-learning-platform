...
,
  ...
  admin: {
    ...
    adapter_source_adaptertype: 'Adapter: {{ source.adapterType }}',
    ...
    added_log_resourcescreated: 'Added {{ log.resourcesCreated }}',
    added_progressmap_source_id: 'Added {{ progressMap[source.id].resourcesCreated }}',
    ...
    aimodelconfigs_length_models_aimodelconfigs: '{{ aiModelConfigs.length }} models · {{ aiModelConfigs.filter(m => m.enabled).length }} enabled',
    ...
    c_durationdays_days: '{{ c.durationDays }} days',
    ...
    charcount_characters: '{{ charCount }} characters',
    ...
    course_count_enrollments_0: '{{ course._count?.enrollments || 0 }} people have participated',
    course_count_reviews_reviews: '{{ course._count.reviews }} Reviews',
    ...
    course_lessons_length_0: '{{ course.lessons?.length || 0 }} Lessons',
    ...
    current_model_temperature_tofixed: 'Current: {{ model.temperature?.toFixed(1) ?? \'0.7\' }}',
    ...
    delete_log_resourcesdeleted: 'Delete {{ log.resourcesDeleted }}',
    ...
    found_progressmap_source_id: 'Found {{ progressMap[source.id].resourcesFound }}',
    ...
    iscleaning_cleaning_clean_now: '{{ isCleaning ? \'Cleaning...\' : \'Clean now\' }}',
    ...
    issaving_saving_saving_global_1: '{{ isSaving ? \'Saving...\' : \'Saving global settings\' }}',
    istestingai_testing_testing_the: '{{ isTestingAi ? \'Testing...\' : \'Testing the default model connection\' }}',
    istestingai_testingaimodelid_model_id: '{{ isTestingAi && testingAiModelId === model.id ? \'Testing\' : \'Testing\' }}',
    ...
    linecount_lines: '{{ lineCount }} lines',
    link: 'link',
    link_broadcast_link: 'Link: {{ broadcast.link }}',
    ...
    log_out: 'Log out',
    log_resourcesfound_found: '{{ log.resourcesFound }} found',
    log_user_name_system: '{{ log.user?.name || \'System\' }}',
    login: 'Login',
    ...
    microsoftpoolstats_active: '{{ microsoftPoolStats.active }}',
    microsoftpoolstats_activewithproxy: '{{ microsoftPoolStats.activeWithProxy }}',
    microsoftpoolstats_total: '{{ microsoftPoolStats.total }}',
    ...
    model_name_model_modelname: '{{ model.name || model.modelName || \'Unnamed model\' }}',
    model_pool_configuration: 'Model pool configuration',
    model_showadvanced_hide_advanced: '{{ model.showAdvanced ? \'Hide advanced parameters\' : \'Expand advanced parameter settings\' }}',
    ...
    overdue: 'Overdue',
    page_pagination_page_pagination: 'Page {{ pagination.page }} / {{ pagination.totalPages }}',
    page_progressmap_source_id: 'Page {{ progressMap[source.id].currentPage }}',
    ...
    permissions_getplanname_source_minplanpriority: 'Permissions: {{ getPlanName(source.minPlanPriority) }} or above',
    ...
    plan_price_month_1: '￥{{ plan.price }}/month',
    plan_subscribercount_0_subscribers: '{{ plan.subscriberCount || 0 }} Subscribers',
    plane: 'Plane',
    planform_ispopular_recommended_normal: '{{ planForm.isPopular ? \'recommended\' : \'normal\' }}',
    ...
    project: 'Project',
    proxy_account_proxy_split: '🔒 Proxy: {{ account.proxy.split(\'@\').pop() }}',
    ...
    selected_files_excelfiles_length: 'Selected files ({{ excelFiles.length }})',
    selectedids_length_records_have: '{{ selectedIds.length }} records have been checked',
    ...
    settings: 'settings',
    settings_ai_import_enabled: '{{ settings.AI_IMPORT_ENABLED ? \'✓ Function activated\' : \'Function not enabled\' }}',
    showing_resourcepage_1_resourcepagesize: 'Showing {{ (resourcePage - 1) * resourcePageSize + 1 }} to {{ Math.min(resourcePage * resourcePageSize, resourceTotal) }}, a total of {{ resourceTotal }} resources',
    ...
    sorting_weight_cat_order: 'Sorting weight: {{ cat.order || 0 }}',
    ...
    sub_interval_yearly_annual: '{{ sub.interval === \'YEARLY\' ? \'Annual payment\' : \'Monthly payment\' }}',
    subform_autorenew_opened_close: '{{ subForm.autoRenew ? \'Opened\' : \'Close\' }}',
    ...
    super_editor_management_pageconfig: '`Super editor management - ${pageConfig.title.replace(\'center\', \'\')}`',
    super_editor_management_pageconfig_1: 'Super editor management - ${pageConfig.title.replace(\'center\', \'\')}',
    supports_multiple_selection_only: 'Supports multiple selection, only .xlsx format files',
    svip_and_above: 'SVIP and above',
    sync_interval_math_floor: 'Sync interval: {{ Math.floor(source.syncInterval / 60) }} minutes',
    sync_log: 'Sync log',
    ...
    tab_name_replace_audit: '{{ tab.name.replace(\'Audit\', \'\') }}',
    team: 'team',
    ...
    there_are_also_roadmap: 'There are also {{ roadmap.steps.length - 3 }} advanced nodes',
    ...
    total_resourcetotal: 'Total {{ resourceTotal }}',
    total_sourcecategories_length_categories: 'Total {{ sourceCategories.length }} categories',
    ...
    update_log_resourcesupdated: 'Update {{ log.resourcesUpdated }}',
    update_progressmap_source_id: 'Update {{ progressMap[source.id].resourcesUpdated }}',
    ...
    update_time_roadmap_createdat: 'Update time: {{ roadmap.createdAt ? new Date(roadmap.createdAt).toLocaleDateString() : \'-\' }}',
    ...
    used_formatelapsed_progressmap_source: 'Used {{ formatElapsed(progressMap[source.id].startedAt) }}',
    user: 'User',
    user_1: 'User',
    ...
    you_have_switched_the: '⚠️ You have switched the sending mode to "{{ settings.SYSTEM_EMAIL_PROVIDER === \'MICROSOFT_POOL\' ? \'Microsoft Account Pool\' : \'Standard SMTP\' }}", this change has not yet taken effect. Please be sure to click the "Save Global Settings" button in the upper right corner of the page!',
