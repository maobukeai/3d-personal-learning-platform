...
import { dropdownContextKey } from '@/components/ui/dropdownContext';
...
const handleSwitchWorkspace = (ws: Workspace, event?: Event) => {
  if (event) {
    ...
    if (
      target &&
      (target.closest('[data-action="pin"]') || target.closest('[data-action="settings"]'))
    ) {
      return;
    }
  }
...
<template>
  ...
  <DropdownMenuRoot v-else-if="workspaceStore.currentWorkspace" v-model:open="isDropdownOpen">
    <DropdownMenuTrigger
      class="workspace-switcher-badge min-h-8 h-8 px-2.5 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200/50 dark:border-white/5 flex items-center gap-2 cursor-pointer hover:bg-slate-200/50 dark:hover:bg-white/8 transition-colors animate-in fade-in duration-300 min-w-0 outline-none"
    >
      <div class="relative shrink-0 flex items-center justify-center">
        <img
          ...
          class="w-6 h-6 rounded object-cover border border-slate-200/50 dark:border-white/10"
        />
        <div
          v-else
          ...
          <div class="flex items-center justify-between w-full">
            ...
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                "
                :title="
                  authStore.user?.defaultWorkspaceId === ws.id ? '默认工作区' : '设为默认工作区'
                "
