...
<template>
  <div class="flex-1 flex flex-col h-full overflow-hidden transition-colors duration-300" style="background-color: var(--bg-app)">
    ...
    <!-- Tabs -->
    <div class="px-8 mt-4">
      <div class="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl w-fit border border-slate-200 dark:border-slate-700">
        <button 
          @click="activeTab = 'submit'"
          class="flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-bold transition-all"
          :class="activeTab === 'submit' ? 'bg-white dark:bg-slate-700 text-rose-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'"
        >
        ...
        <button 
          ...
          class="flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-bold transition-all"
          :class="activeTab === 'history' ? 'bg-white dark:bg-slate-700 text-rose-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'"
        >
    ...
    <div class="flex-1 overflow-y-auto p-8 scrollbar-hide">
      <div class="max-w-3xl mx-auto">
        ...
        <template v-if="activeTab === 'submit'">
          <div v-if="!isSubmitted" class="rounded-3xl border shadow-sm overflow-hidden transition-all" style="background-color: var(--bg-card); border-color: var(--border-base)">
            <div class="p-8 border-b bg-slate-50/30" style="border-color: var(--border-base)">
              <h2 class="text-lg font-bold mb-2" style="color: var(--text-primary)">遇到麻烦了吗？</h2>
              <p class="text-sm text-slate-500">描述你遇到的问题或改进建议，我们的技术团队会尽快处理。</p>
            </div>
            ...
            <div class="p-8 space-y-6">
              ...
              <div class="grid grid-cols-2 gap-6">
                <div>
                  <label class="block text-xs font-bold text-slate-400 uppercase mb-2">反馈类型</label>
              ...
              <div>
                <label class="block text-xs font-bold text-slate-400 uppercase mb-2">问题摘要</label>
                <el-input v-model="bugForm.title" placeholder="请用一句话简述你遇到的问题..." size="large" />
              </div>

              <div>
                <label class="block text-xs font-bold text-slate-400 uppercase mb-2">详细描述</label>
                <el-input 
              ...
              <!-- Upload Area -->
              <div>
                <label class="block text-xs font-bold text-slate-400 uppercase mb-2">附件/截图 (可选)</label>
                ...
                >
                  <template v-if="!isUploading">
                    <ImageIcon class="w-8 h-8 mb-2 opacity-40 group-hover:text-accent transition-colors" />
                    <p class="text-xs font-bold">点击或拖拽图片到此处上传</p>
                    <p class="text-[10px] mt-1">支持 PNG, JPG, GIF (最大 5MB)</p>
                  </template>
                  <template v-else>
                    <div class="w-8 h-8 border-2 border-accent/30 border-t-accent rounded-full animate-spin mb-2"></div>
                    <p class="text-xs font-bold text-accent">正在上传...</p>
                  </template>
                </div>

                <!-- Image Preview -->
                <div v-else class="relative w-full max-sm rounded-2xl overflow-hidden border border-slate-200 shadow-sm group">
                  <img :src="previewUrl" class="w-full h-auto object-cover" />
                  <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button @click="removeAttachment" class="p-2 bg-rose-500 text-white rounded-full hover:bg-rose-600 transition-all transform hover:scale-110">
                      <X class="w-5 h-5" />
                    </button>
                  </div>
                </div>
          ...
          <!-- Success State -->
          <div v-else class="rounded-3xl border p-12 text-center animate-in fade-in zoom-in duration-500" style="background-color: var(--bg-card); border-color: var(--border-base)">
            <div class="w-20 h-20 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-6">
              <CheckCircle2 class="w-10 h-10" />
            </div>
            <h2 class="text-2xl font-bold mb-2" style="color: var(--text-primary)">报告提交成功！</h2>
            <p class="text-slate-500 mb-8 max-w-sm mx-auto">感谢你为完善平台做出的贡献。你可以继续浏览其他页面，或者在反馈历史中查看处理进度。</p>
            <div class="flex items-center justify-center gap-4">
              <button @click="isSubmitted = false" class="px-6 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-bold text-xs hover:bg-slate-50 transition-all">
                再次提交
              </button>
              <RouterLink to="/dashboard" class="px-6 py-2.5 rounded-xl bg-slate-800 text-white font-bold text-xs hover:bg-slate-900 transition-all shadow-lg shadow-slate-200">
                返回仪表盘
              </RouterLink>
            </div>
          </div>
        ...
        <template v-else>
          <div class="space-y-4">
            <div v-if="isLoadingHistory" class="flex flex-col items-center justify-center py-20 text-slate-400">
              <div class="w-8 h-8 border-2 border-rose-500/30 border-t-rose-500 rounded-full animate-spin mb-4"></div>
            ...
            <template v-else>
              <div v-for="item in myFeedbacks" :key="item.id" 
                ...
                <div class="flex items-start justify-between mb-3">
                  ...
                  <el-tag :type="getStatusType(item.status)" size="small" effect="light" class="font-bold">
                ...
                <div class="flex items-center gap-4 text-[10px] text-slate-400 border-t border-slate-50 dark:border-slate-800 pt-3">
              ...
              <div v-if="myFeedbacks.length === 0" class="flex flex-col items-center justify-center py-20 text-slate-400 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-dashed border-slate-200 dark:border-slate-700">
                <History class="w-12 h-12 opacity-10 mb-4" />
                <p class="text-sm font-medium">还没有任何反馈记录</p>
                <button @click="activeTab = 'submit'" class="mt-4 text-xs font-bold text-rose-600 hover:underline">去提交第一个反馈</button>
              </div>
            </template>
          </div>
        </template>
        ...
        <div class="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="bg-accent-subtle/50 p-6 rounded-2xl border border-accent-subtle/50">
            ...
            <ul class="space-y-3">
              <li v-for="i in 3" :key="i" class="flex items-start gap-2 text-xs text-accent/70 hover:text-accent cursor-pointer group">
                <ChevronRight class="w-3.5 h-3.5 shrink-0 group-hover:translate-x-1 transition-transform" />
          ...
          <div class="bg-slate-800 p-6 rounded-2xl text-white">
            ...
            <p class="text-xs text-slate-400 leading-relaxed mb-4">如果你遇到了紧急的账号安全问题，请直接拨打我们的 24/7 技术支持热线或通过即时聊天联系我们。</p>
            <button class="text-xs font-bold text-accent hover:text-accent/30 transition-colors flex items-center gap-1">
