...
import { useResourceComments } from '@/composables/useResourceComments'; /** * Inline discussions block for MaterialDetailPanel. Owns the comments * composable so the parent no longer has to thread comment state through; * fetches whenever the bound material id changes. */
...
const {
  ...
  handleDeleteComment,
  canDeleteComment,
} = useResourceComments('materials', () => props.resourceId);
watch(
  () => props.resourceId,
  (id) => {
    if (id) fetchComments();
