...
import {
  Search, FolderPlus, LayoutGrid, List, MoreHorizontal,
  Clock, Layers, CheckCircle2, X, TrendingUp, Activity,
  Award, Users, Plus, Calendar, KanbanSquare,
  FolderOpen, Tag, Zap, ArrowUp, Minus, ArrowDown, Flame,
  AlertCircle, BarChart3, Briefcase
...
const taskStats = computed(() => {
  ...
  const overdue = tasks.value.filter(t => t.dueDate && new Date(t.dueDate) < new Date() && t.status !== 'DONE').length
...
const filteredTasks = computed(() => {
  ...
  if (dateFilter.value !== 'all') {
    ...
    if (dateFilter.value === 'overdue') {
      ...
      filtered = filtered.filter(t => t.dueDate && new Date(t.dueDate) >= now && new Date(t.dueDate) <= endOfDay)
    } else if (dateFilter.value === 'week') {
      const endOfWeek = new Date(now)
      endOfWeek.setDate(now.getDate() + 7)
      filtered = filtered.filter(t => t.dueDate && new Date(t.dueDate) >= now && new Date(t.dueDate) <= endOfWeek)
    }
  }
...
const fetchAll = async () => {
  ...
  await Promise.all([fetchProjects(), fetchTasks(), fetchStats(), fetchTeamMembers(), fetchTeams()])
...
const openTaskDialog = () => {
  taskForm.value = { title: '', description: '', assigneeId: '', dueDate: '', priority: 'MEDIUM', projectId: '', teamId: workspaceStore.activeTeamId || '', participantIds: [] }
...
const handleCreateTask = async () => {
  ...
  try {
    const payload = {
      ...
      participantIds: taskForm.value.participantIds.length > 0 ? taskForm.value.participantIds : undefined,
    }
  ...
  <div class="flex-1 flex flex-col h-full overflow-hidden" style="background-color: var(--bg-app)">
    ...
    <div class="h-20 px-10 flex items-center justify-between shrink-0 bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl border-b z-10 sticky top-0" style="border-color: var(--border-base)">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-2xl font-black tracking-tight" style="color: var(--text-primary)">团队任务</h1>
          <p class="text-[10px] font-bold uppercase tracking-widest text-slate-400">管理团队项目与协作任务</p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-6">
        ...
        <div class="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
          <button
            ...
            :class="activeTab === 'projects' ? 'bg-white dark:bg-slate-700 text-accent shadow-sm' : 'text-slate-400 hover:text-slate-600'"
          >
            <Layers class="w-4 h-4" /> 项目
          </button>
          <button
            @click="activeTab = 'tasks'"
            class="flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all"
            :class="activeTab === 'tasks' ? 'bg-white dark:bg-slate-700 text-accent shadow-sm' : 'text-slate-400 hover:text-slate-600'"
          >
            <KanbanSquare class="w-4 h-4" /> 任务看板
          </button>
        </div>

        <button v-if="activeTab === 'projects'" @click="openAddDrawer" class="group relative px-6 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-bold overflow-hidden shadow-xl shadow-slate-900/10 dark:shadow-white/10 hover:scale-105 active:scale-95 transition-all flex items-center gap-2">
        ...
        <button v-else @click="openTaskDialog" class="flex items-center gap-2 px-6 py-3 bg-accent text-white rounded-2xl text-xs font-bold hover:shadow-lg hover:shadow-accent/20 transition-all">
    ...
    <div class="flex-1 overflow-y-auto scrollbar-hide">
      ...
      <template v-else>
        ...
        <div v-show="activeTab === 'projects'" class="p-10">
          <div class="max-w-[1600px] mx-auto space-y-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div class="bg-white dark:bg-slate-900 rounded-[2rem] p-6 border shadow-sm flex items-center gap-6" style="border-color: var(--border-base)">
                ...
                <div>
                  ...
                  <h3 class="text-3xl font-black" style="color: var(--text-primary)">{{ projectStats.total }}</h3>
                </div>
              </div>
              <div class="bg-white dark:bg-slate-900 rounded-[2rem] p-6 border shadow-sm flex items-center gap-6" style="border-color: var(--border-base)">
                ...
                <div>
                  ...
                  <h3 class="text-3xl font-black" style="color: var(--text-primary)">{{ projectStats.active }}</h3>
                </div>
              </div>
              <div class="bg-white dark:bg-slate-900 rounded-[2rem] p-6 border shadow-sm flex items-center gap-6" style="border-color: var(--border-base)">
                ...
                <div>
                  ...
                  <h3 class="text-3xl font-black" style="color: var(--text-primary)">{{ projectStats.completed }}</h3>
                </div>
              </div>
              <div class="bg-white dark:bg-slate-900 rounded-[2rem] p-6 border shadow-sm flex items-center gap-6" style="border-color: var(--border-base)">
                ...
                <div>
                  ...
                  <div class="flex items-baseline gap-2">
                    <h3 class="text-3xl font-black" style="color: var(--text-primary)">{{ projectStats.completionRate }}</h3>
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-4">
                <div class="flex items-center bg-white dark:bg-slate-900 rounded-xl border p-1 shadow-sm" style="border-color: var(--border-base)">
                  <button @click="viewMode = 'grid'" class="p-2 rounded-lg transition-all" :class="viewMode === 'grid' ? 'bg-slate-100 dark:bg-slate-800 text-accent shadow-sm' : 'text-slate-400 hover:text-slate-600'">
                    <LayoutGrid class="w-4 h-4" />
                  </button>
                  <button @click="viewMode = 'list'" class="p-2 rounded-lg transition-all" :class="viewMode === 'list' ? 'bg-slate-100 dark:bg-slate-800 text-accent shadow-sm' : 'text-slate-400 hover:text-slate-600'">
                    <List class="w-4 h-4" />
                  </button>
                </div>
              </div>
            ...
            <div v-if="filteredProjects.length === 0" class="flex flex-col items-center justify-center py-32 text-center bg-white/50 dark:bg-slate-900/50 rounded-[3rem] border border-dashed" style="border-color: var(--border-base)">
            ...
            <div v-else-if="viewMode === 'grid'" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
              <div v-for="project in filteredProjects" :key="project.id"
                   ...
                   class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group cursor-pointer relative overflow-hidden"
                ...
                <div class="relative z-10 flex flex-col h-full">
                  ...
                  <h3 class="text-lg font-black group-hover:text-accent transition-colors mb-2 line-clamp-1" style="color: var(--text-primary)">{{ project.title }}</h3>
                  <p class="text-xs leading-relaxed mb-6 line-clamp-2" style="color: var(--text-secondary)">{{ project.description || '暂无项目描述。' }}</p>
                  <div class="flex flex-wrap gap-2 mb-8">
                    <span v-for="tag in getTagsList(project.tags).slice(0,3)" :key="tag" class="text-[10px] font-black px-2.5 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-slate-500">#{{ tag }}</span>
                    <span v-if="getTagsList(project.tags).length > 3" class="text-[10px] font-black px-2.5 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-slate-500">+{{ getTagsList(project.tags).length - 3 }}</span>
                  </div>
                  <div class="mt-auto space-y-3 mb-8">
                    ...
                    <div class="h-2 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800">
                      <div class="h-full rounded-full transition-all duration-1000" :class="project.progress === 100 ? 'bg-emerald-500' : 'bg-accent'" :style="{ width: project.progress + '%' }"></div>
                    </div>
                  </div>
            ...
            <div v-else class="bg-white dark:bg-slate-900 rounded-[2.5rem] border shadow-sm overflow-hidden" style="border-color: var(--border-base)">
                ...
                <table class="w-full text-left border-collapse">
                  ...
                  <tbody>
                    <tr v-for="project in filteredProjects" :key="project.id"
                      ...
                      <td class="px-8 py-6">
                        <div class="flex items-center gap-4">
                          <div class="w-12 h-12 rounded-2xl flex items-center justify-center text-white text-lg font-black shadow-sm" :class="project.color">{{ project.title.substring(0, 1) }}</div>
                          <div>
                            <p class="text-sm font-black group-hover:text-accent transition-colors" style="color: var(--text-primary)">{{ project.title }}</p>
        ...
        <div v-show="activeTab === 'tasks'" class="p-10">
          <div class="max-w-[1600px] mx-auto space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div class="bg-white dark:bg-slate-900 rounded-[2rem] p-6 border shadow-sm flex items-center gap-6" style="border-color: var(--border-base)">
                ...
                <div>
                  <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">总任务数</p>
                  <h3 class="text-3xl font-black" style="color: var(--text-primary)">{{ taskStats.total }}</h3>
                </div>
              </div>
              <div class="bg-white dark:bg-slate-900 rounded-[2rem] p-6 border shadow-sm flex items-center gap-6" style="border-color: var(--border-base)">
                <div class="w-14 h-14 rounded-2xl bg-emerald-500/10 flex items-center justify-center shrink-0">
                ...
                <div>
                  <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">已完成</p>
                  <h3 class="text-3xl font-black" style="color: var(--text-primary)">{{ taskStats.done }}</h3>
                </div>
              </div>
              <div v-if="taskStats.overdue > 0" class="bg-white dark:bg-slate-900 rounded-[2rem] p-6 border shadow-sm flex items-center gap-6" style="border-color: var(--border-base)">
                ...
                <div>
                  <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">逾期任务</p>
                  <h3 class="text-3xl font-black text-rose-500">{{ taskStats.overdue }}</h3>
                </div>
              </div>
              <div class="bg-white dark:bg-slate-900 rounded-[2rem] p-6 border shadow-sm flex items-center gap-6" style="border-color: var(--border-base)">
                <div class="w-14 h-14 rounded-2xl bg-purple-500/10 flex items-center justify-center shrink-0">
                ...
                <div>
                  <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">完成率</p>
                  <div class="flex items-baseline gap-2">
                    <h3 class="text-3xl font-black" style="color: var(--text-primary)">{{ taskStats.completionRate }}</h3>
                    <span class="text-sm font-bold text-slate-400">%</span>
                  </div>
                </div>
              </div>
            ...
            <div class="flex items-center gap-6 overflow-x-auto scrollbar-hide">
              <div class="flex items-center gap-2">
                ...
                <div class="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
                  <button v-for="f in [{ id: 'all', label: '全部' }, { id: 'overdue', label: '逾期' }, { id: 'today', label: '今日' }, { id: 'week', label: '本周' }]" :key="f.id"
              ...
              <div class="flex items-center gap-2">
                ...
                <div class="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
                  <button @click="priorityFilter = 'all'" class="px-3 py-1 rounded-lg text-[10px] font-bold transition-all" :class="priorityFilter === 'all' ? 'bg-white dark:bg-slate-700 text-accent shadow-sm' : 'text-slate-500 hover:text-slate-700'">全部</button>
                  <button v-for="p in priorityOptions" :key="p.id" @click="priorityFilter = p.id"
            ...
            <div class="flex gap-6 min-h-[500px]">
              <div v-for="col in columns" :key="col.id" class="flex-1 flex flex-col min-w-[300px] rounded-2xl transition-colors duration-300 overflow-hidden" style="background-color: var(--bg-card)">
                ...
                <div class="flex-1 overflow-y-auto space-y-3 px-4 pb-4 scrollbar-hide min-h-[100px]">
                  <div v-for="task in (tasksByStatus as Record<string, any[]>)[col.id]" :key="task.id"
                       class="group p-4 rounded-xl border shadow-sm hover:shadow-md hover:border-accent/30 transition-all cursor-pointer relative"
                  ...
                  <div v-if="(tasksByStatus as Record<string, any[]>)[col.id].length === 0"
                    ...
                    <p class="text-[10px] font-bold">暂无任务</p>
                  </div>
                </div>
              </div>
            </div>
    ...
    <el-drawer v-model="isDrawerOpen" :title="isEditMode ? '项目设定' : '启动新项目'" size="500px" :show-close="false" class="custom-drawer">
      <template #header="{ close }">
        <div class="flex items-center justify-between">
          <h3 class="text-2xl font-black tracking-tight" style="color: var(--text-primary)">{{ isEditMode ? '配置工作流' : '构思新世界' }}</h3>
      ...
      <div class="space-y-8 p-1">
        ...
        <div class="grid grid-cols-2 gap-6">
          <div>
            ...
            <el-date-picker v-model="projectForm.dueDate" type="date" placeholder="选择 Deadline" class="!w-full !rounded-2xl custom-date-picker" />
          </div>
          <div>
      ...
      <template #footer>
        <div class="flex gap-4 p-4 border-t" style="border-color: var(--border-base)">
          <button @click="isDrawerOpen = false" class="flex-1 py-4 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-2xl font-black transition-all" style="color: var(--text-primary)">取消操作</button>
          <button @click="handleSaveProject" class="flex-[2] py-4 bg-accent text-white rounded-2xl font-black shadow-xl shadow-accent/20 hover:scale-[1.02] active:scale-[0.98] transition-all">{{ isEditMode ? '确认并应用更改' : '正式启动项目' }}</button>
        </div>
      </template>
    </el-drawer>

    <Transition name="fade">
      <div v-if="isTaskDialogOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="isTaskDialogOpen = false"></div>
        <div class="relative w-full max-w-lg p-8 rounded-3xl shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto" style="background-color: var(--bg-card)">
          <div class="flex items-center justify-between">
            <h3 class="text-xl font-bold" style="color: var(--text-primary)">新建团队任务</h3>
            <button @click="isTaskDialogOpen = false" style="color: var(--text-secondary)"><X class="w-5 h-5" /></button>
          </div>
          <div class="space-y-4">
            <div>
              <label class="block text-xs font-bold uppercase mb-2 ml-1 text-slate-400">任务标题</label>
              <input v-model="taskForm.title" type="text" class="w-full px-4 py-3 bg-slate-100 dark:bg-white/5 border-none rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-accent/20 transition-all" placeholder="例如：完成角色建模" />
            </div>
            <div>
              <label class="block text-xs font-bold uppercase mb-2 ml-1 text-slate-400">详细描述 (可选)</label>
              <textarea v-model="taskForm.description" rows="3" class="w-full px-4 py-3 bg-slate-100 dark:bg-white/5 border-none rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-accent/20 transition-all resize-none" placeholder="任务目标和交付物..."></textarea>
            </div>
            <div>
              <label class="block text-xs font-bold uppercase mb-2 ml-1 text-slate-400">所属团队</label>
              <el-select v-model="taskForm.teamId" placeholder="选择团队" class="!w-full custom-select" @change="onTaskTeamChange">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <el-date-picker v-model="taskForm.dueDate" type="date" placeholder="选择截止日期" class="!w-full !rounded-2xl custom-date-picker" popper-class="custom-date-popper" />
              </div>
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <el-select v-model="taskForm.projectId" clearable placeholder="选择项目" class="!w-full custom-select">
          ...
          <button @click="handleCreateTask" class="w-full py-4 bg-accent text-white rounded-2xl font-bold shadow-lg shadow-accent/20 hover:shadow-accent/40 transition-all">创建任务</button>
        </div>
      </div>
    </Transition>
...
:deep(.el-drawer__header) { margin-bottom: 0 !important; padding: 2rem !important; border-bottom: 1px solid var(--border-base); }
:deep(.el-drawer__body) { padding: 2rem !important; }
:deep(.el-drawer__footer) { padding: 0 !important; }
...
.custom-slider :deep(.el-slider__bar) { background-color: var(--el-color-primary); }
.custom-slider :deep(.el-slider__button) { border-color: var(--el-color-primary); width: 20px; height: 20px; }
</style>
...
.custom-date-popper .el-picker-panel { border-radius: 1.5rem !important; border: none !important; }
.custom-date-popper .el-popper__arrow::before { border: 1px solid var(--border-base) !important; }
</style>
