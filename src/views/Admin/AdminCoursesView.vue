...
const parseSceneConfig = (config: any) => {
  if (!config) return { environment: 'sunset', exposure: 1.0, lights: { intensity: 1.0, color: '#ffffff' } }
  if (typeof config === 'string') {
    try { return JSON.parse(config) } catch { return { environment: 'sunset', exposure: 1.0, lights: { intensity: 1.0, color: '#ffffff' } } }
  }
  return config
}
...
const parseHotspots = (hotspots: any) => {
  ...
  if (typeof hotspots === 'string') {
    try { return JSON.parse(hotspots) } catch { return [] }
  }
  return hotspots
}
...
const captureCameraForHotspot = () => {
  ...
  if (state) {
    ...
    ElMessage.success(`已保存当前视角到热点: ${lessonForm.value.hotspots[currentHotspotIndex.value].title}`)
  }
}
...
const openLessonModal = (course: any, lesson: any = null) => {
  ...
  if (lesson) {
    ...
    lessonForm.value = {
      ...
      duration: 0,
      hotspots: [],
      sceneConfig: { environment: 'sunset', exposure: 1.0, lights: { intensity: 1.0, color: '#ffffff' } }
    }
  }
...
<template>
  <div class="flex-1 flex flex-col h-full overflow-hidden transition-colors duration-300" style="background-color: var(--bg-app)">
          ...
          <div>
            ...
            <div class="flex gap-2">
              <input v-model="lessonForm.videoUrl" type="text" placeholder="https://..."
                     ...
                     style="background-color: var(--bg-app); border-color: var(--border-base); color: var(--text-primary)" />
              <button v-if="is3DModel(lessonForm.videoUrl)" @click="openHotspotEditor"
              ...
              <button v-if="is3DModel(lessonForm.videoUrl)" @click="openSceneSettings"
            ...
            <p v-if="lessonForm.hotspots.length > 0" class="text-[10px] text-emerald-500 font-bold mt-1 ml-1">
    ...
    <el-dialog v-model="isHotspotEditorOpen" title="3D 交互热点编辑器" fullscreen class="hotspot-editor-dialog">
      <div class="h-full flex flex-col">
        <div class="flex-1 flex gap-6 overflow-hidden">
          ...
          <div class="flex-1 bg-slate-900 rounded-3xl overflow-hidden relative">
            ...
            />
            <div class="absolute top-6 left-6 bg-black/40 backdrop-blur-md px-4 py-2 rounded-xl border border-white/10 pointer-events-none">
            ...
            <div class="absolute bottom-6 left-6 flex items-center gap-3">
              <button @click="captureCameraForHotspot" v-if="currentHotspotIndex !== -1"
          ...
          <div class="w-80 flex flex-col gap-4 overflow-hidden">
            <div class="bg-white dark:bg-slate-900 rounded-3xl border p-6 flex-1 flex flex-col overflow-hidden" style="border-color: var(--border-base)">
              <h4 class="font-black text-sm mb-4 flex items-center gap-2" style="color: var(--text-primary)">
              ...
              <div class="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3">
                <div v-for="(hs, index) in lessonForm.hotspots" :key="index"
                     @click="currentHotspotIndex = index"
                     class="p-4 rounded-2xl border transition-all hover:border-accent/40 group cursor-pointer"
                     :class="currentHotspotIndex === index ? 'border-accent bg-accent/5' : 'bg-slate-50 dark:bg-white/5'"
                     style="border-color: var(--border-base)">
                  <div class="flex items-start justify-between mb-2">
                    <div class="flex items-center gap-2">
                      <span class="w-6 h-6 rounded-lg bg-accent text-white flex items-center justify-center text-[10px] font-black">{{ index + 1 }}</span>
                      <Camera v-if="hs.cameraPos" class="w-3.5 h-3.5 text-emerald-500" title="已设置视角" />
                    </div>
                    <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button @click.stop="startEditHotspot(index)" class="p-1.5 rounded-lg text-slate-400 hover:text-accent">
                        <Edit2 class="w-3.5 h-3.5" />
                      </button>
                      <button @click.stop="removeHotspot(index)" class="p-1.5 rounded-lg text-slate-400 hover:text-rose-500">
                        <Trash2 class="w-3.5 h-3.5" />
                      </button>
                    </div>
                  </div>
                  
                  <h5 class="text-xs font-bold mb-1 truncate" style="color: var(--text-primary)">{{ hs.title }}</h5>
                  <p class="text-[10px] text-slate-400 line-clamp-2 leading-relaxed">{{ hs.content }}</p>
                </div>

                <div v-if="lessonForm.hotspots.length === 0" class="py-12 text-center">
                  <div class="w-12 h-12 rounded-full bg-slate-50 dark:bg-white/5 flex items-center justify-center mx-auto mb-3">
                    <Plus class="w-6 h-6 text-slate-200" />
                  </div>
                  <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">暂无热点</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          <button @click="isHotspotEditorOpen = false; currentHotspotIndex = -1" class="px-8 py-2.5 rounded-xl border font-bold text-sm hover:bg-slate-50 transition-colors">关闭并应用</button>
        </div>
      </template>
    </el-dialog>

    <!-- Hotspot Quick Edit Modal -->
    <el-dialog v-slot:default v-model="isQuickEditOpen" title="编辑热点详情" width="400px" append-to-body>
      <div class="space-y-4">
        <div>
          <label class="block text-xs font-bold text-slate-400 mb-2">热点标题</label>
          <input v-model="hotspotEditForm.title" type="text" class="w-full px-4 py-2 rounded-xl border outline-none" />
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-400 mb-2">描述内容</label>
          <textarea v-model="hotspotEditForm.content" rows="4" class="w-full px-4 py-2 rounded-xl border outline-none resize-none text-sm"></textarea>
        </div>
        <div class="flex gap-2">
          <button @click="isQuickEditOpen = false" class="flex-1 py-2 rounded-xl border font-bold text-xs">取消</button>
          <button @click="saveHotspotEdit" class="flex-1 py-2 bg-accent text-white rounded-xl font-bold text-xs">保存</button>
        </div>
      </div>
    </el-dialog>

    <!-- Scene Settings Dialog -->
    <el-dialog v-model="isSceneSettingsOpen" title="3D 场景与灯光设置" width="460px" class="custom-rounded-dialog">
      <div class="space-y-6">
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-xs font-black uppercase tracking-widest text-slate-400 mb-3 ml-1">渲染环境 (HDR)</label>
          ...
          <div>
            <div class="flex items-center justify-between mb-2 ml-1">
              <label class="text-xs font-black uppercase tracking-widest text-slate-400">曝光强度 (Exposure)</label>
              <span class="text-xs font-bold text-accent">{{ sceneConfigForm.exposure.toFixed(1) }}</span>
            </div>
          ...
          <div>
            <div class="flex items-center justify-between mb-2 ml-1">
              <label class="text-xs font-black uppercase tracking-widest text-slate-400">主灯光强度</label>
              <span class="text-xs font-bold text-accent">{{ sceneConfigForm.lights.intensity.toFixed(1) }}</span>
            </div>
            <el-slider v-model="sceneConfigForm.lights.intensity" :min="0" :max="5.0" :step="0.1" />
          </div>

          <div>
            <label class="block text-xs font-black uppercase tracking-widest text-slate-400 mb-3 ml-1">灯光色调</label>
            <div class="flex items-center gap-3">
              <el-color-picker v-model="sceneConfigForm.lights.color" />
              <input v-model="sceneConfigForm.lights.color" type="text" 
      ...
      <template #footer>
        <div class="flex gap-3 pt-4">
          <button @click="isSceneSettingsOpen = false" class="flex-1 py-3 border border-slate-200 rounded-2xl font-bold text-sm hover:bg-slate-50 transition-colors">取消</button>
          <button @click="saveSceneSettings" 
